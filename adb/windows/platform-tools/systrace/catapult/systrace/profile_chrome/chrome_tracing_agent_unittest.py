...
class ChromeAgentTest(agents_unittest.BaseAgentTest):
  ...
  def testGetCategories(self):
    ...
    categories = \
        chrome_tracing_agent.ChromeTracingAgent.GetCategories(
            self.presentation.device, self.package_info)
  ...
  def testTracing(self):
    ...
    categories = '*'
    ring_buffer = False
    agent = chrome_tracing_agent.ChromeTracingAgent(self.presentation.device,
                                                    self.package_info,
                                                    ring_buffer)
    agent.StartAgentTracing(chrome_tracing_agent.ChromeConfig(categories, None,
        None, None, None, None, None, None, ring_buffer, self.presentation.device,
        self.package_info))
