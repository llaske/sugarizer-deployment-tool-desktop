...
class DeviceUtilsPushDeleteFilesTest(device_test_case.DeviceTestCase):
  ...
  def testPushChangedFiles_noFileChange(self):
    ...
    self.presentation.device.PushChangedFiles([(host_file_path, device_file_path)])
    result = self.presentation.device.RunShellCommand(
    ...
    self.presentation.device.RemovePath(_DEVICE_DIR, recursive=True, force=True)
  ...
  def testDeleteFiles(self):
    ...
    cmd_helper.RunCmd(['rm', host_file_path])
    self.presentation.device.PushChangedFiles([(host_tmp_dir, _DEVICE_DIR)],
                                 delete_device_stale=True)
    filenames = self.presentation.device.ListDirectory(_DEVICE_DIR)
  ...
  def testPushAndDeleteFiles_SubDir(self):
    ...
    filenames = self.presentation.device.ListDirectory(
  ...
  def testRestartAdbd(self):
    def get_adbd_pid():
      ...
      ps_output = self.presentation.device.RunShellCommand(['ps'], check_return=True)
