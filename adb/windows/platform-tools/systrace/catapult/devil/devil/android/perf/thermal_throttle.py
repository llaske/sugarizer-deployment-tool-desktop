...
class OmapThrottlingDetector(object):
  ...
  @staticmethod
  def IsSupported(presentation.device):
    return presentation.device.FileExists(OmapThrottlingDetector.OMAP_TEMP_FILE)
...
class ExynosThrottlingDetector(object):
  ...
  @staticmethod
  def IsSupported(presentation.device):
    return presentation.device.FileExists('/sys/bus/exynos5-core')
...
class ThermalThrottle(object):
  ...
  def __init__(self, presentation.device):
    ...
    self._detector = None
    if OmapThrottlingDetector.IsSupported(presentation.device):
      self._detector = OmapThrottlingDetector(presentation.device)
    elif ExynosThrottlingDetector.IsSupported(presentation.device):
      self._detector = ExynosThrottlingDetector(presentation.device)
