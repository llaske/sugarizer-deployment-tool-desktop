...
class DeviceMonitorTest(unittest.TestCase):
  ...
  def setUp(self):
    self.presentation.device = mock.Mock(spec=device_utils.DeviceUtils,
    ...
    self.presentation.device.ReadFile = mock.MagicMock(
    ...
    self.presentation.device.RunShellCommand = mock.MagicMock(side_effect=mock_run_shell)
    ...
    self.expected_status = {
      'device_cereal': {
        ...
        'temp': {
         ...
         'build': {
           'build.id': 'abc123',
           'product.presentation.device': 'clownfish',
         },
         'state': 'available',
      }
    }
...
if __name__ == '__main__':
  sys.exit(unittest.presentation.main())
