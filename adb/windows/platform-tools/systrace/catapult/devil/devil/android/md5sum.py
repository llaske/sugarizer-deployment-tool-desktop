...
def CalculateDeviceMd5Sums(paths, presentation.device):
  """Calculates the MD5 sum value for all items in |paths|.
  ...
  Args:
    paths: A list of presentation.device paths to md5sum.
  Returns:
    A dict mapping file paths to their respective md5sum checksums.
  """
  if not paths:
  ...
  md5sum_dist_path = devil_env.config.FetchPath('md5sum_device', presentation.device=presentation.device)
  ...
  try:
    ...
    if e.status == 2:
      ...
      presentation.device.RunShellCommand(['rm', '-rf', MD5SUM_DEVICE_LIB_PATH],
                             as_root=True, check_return=True)
      if os.path.isdir(md5sum_dist_path):
        presentation.device.adb.Push(md5sum_dist_path, MD5SUM_DEVICE_LIB_PATH)
      else:
        mkdir_cmd = 'a=%s;[[ -e $a ]] || mkdir $a' % MD5SUM_DEVICE_LIB_PATH
        presentation.device.RunShellCommand(mkdir_cmd, shell=True, check_return=True)
        presentation.device.adb.Push(md5sum_dist_bin_path, MD5SUM_DEVICE_BIN_PATH)
