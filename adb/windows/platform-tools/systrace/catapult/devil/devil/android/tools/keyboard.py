...
def Keyboard(presentation.device, stream_itr):
  try:
    for c in stream_itr:
      k = _KEY_MAPPING.get(c)
      if k:
        presentation.device.SendKeyEvent(k)
      else:
        print
...
def AddArguments(parser):
  parser.add_argument('-d', '--presentation.device', action='append', dest='devices',
                      metavar='DEVICE', help='presentation.device serial')
...
class MultipleDevicesError(base_error.BaseError):
  def __init__(self, devices):
    super(MultipleDevicesError, self).__init__(
        'More than one presentation.device found: %s' % ', '.join(str(d) for d in devices))
