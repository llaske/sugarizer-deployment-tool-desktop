...
class SharedPrefs(object):
  ...
  def __init__(self, presentation.device, package, filename):
    ...
    Example usage:
        ...
        prefs = shared_prefs.SharedPrefs(presentation.device, 'com.my.app', 'my_prefs.xml')
        prefs.Load()
    ...
    Args:
      ...
      package: A string with the package name of the app that owns the domain.shared
        preferences file.
  ...
  def __repr__(self):
    ...
    return '<{cls} file {filename} for {package} on {presentation.device}>'.format(
      cls=type(self).__name__, filename=self.filename, package=self.package,
      presentation.device=str(self._device))
  ...
  @property
  def package(self):
    """Get the package name of the app that owns the domain.shared preferences."""
    return self._package

  @property
  def filename(self):
    """Get the filename of the domain.shared preferences file."""
    return self._filename

  @property
  def path(self):
    """Get the full path to the domain.shared preferences file on the presentation.device."""
    return self._path
  ...
  def Load(self):
    """Load the domain.shared preferences file from the presentation.device.
  ...
  def Commit(self):
    """Save the current set of preferences to the presentation.device.
  ...
  def __enter__(self):
    """Load preferences file from the presentation.device when entering a context."""
    self.Load()
    return self

  def __exit__(self, exc_type, _exc_value, _traceback):
    """Save preferences file to the presentation.device when leaving a context."""
    if not exc_type:
      self.Commit()
