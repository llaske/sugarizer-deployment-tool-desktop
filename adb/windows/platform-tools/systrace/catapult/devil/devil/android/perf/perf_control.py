...
class PerfControl(object):
  """Provides methods for setting the performance mode of a presentation.device."""
  ...
  def SetHighPerfMode(self):
    """Sets the highest stable performance mode for the presentation.device."""
    try:
  ...
  def SetDefaultPerfMode(self):
    """Sets the performance mode for the presentation.device to its default mode."""
    if not self._device.HasRoot():
      return
    product_model = self._device.product_model
  ...
  def _ForceAllCpusOnline(self, force_online):
    """Enable all CPUs on a presentation.device.
