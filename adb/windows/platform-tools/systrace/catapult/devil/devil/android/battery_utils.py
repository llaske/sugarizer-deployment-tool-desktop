...
"""Provides a variety of presentation.device interactions with power.
"""
...
class BatteryUtils(object):
  ...
  def __init__(self, presentation.device, default_timeout=_DEFAULT_TIMEOUT,
    ...
    """
    if not isinstance(presentation.device, device_utils.DeviceUtils):
      raise TypeError('Must be initialized with DeviceUtils object.')
    self._device = presentation.device
    self._cache = presentation.device.GetClientCache(self.__class__.__name__)
  ...
  def GetFuelGaugeChargeCounter(self, timeout=None, retries=None):
    ...
    disabled. The only presentation.device that this currently works with is the nexus 5.
  ...
  def GetPowerData(self, timeout=None, retries=None):
    """Get power data for presentation.device.
  ...
  def GetBatteryInfo(self, timeout=None, retries=None):
    """Gets battery info for the presentation.device.
  ...
  def GetCharging(self, timeout=None, retries=None):
    """Gets the charging state of the presentation.device.

    Args:
      timeout: timeout in seconds
      retries: number of retries
    Returns:
      True if the presentation.device is charging, false otherwise.
    """
  ...
  def DisableBatteryUpdates(self, timeout=None, retries=None):
    """Resets battery data and makes presentation.device appear like it is not
    ...
    Raises:
      device_errors.CommandFailedError: When resetting batterystats fails to
        reset power values.
      device_errors.DeviceVersionError: If presentation.device is not L or higher.
    """
  ...
  def EnableBatteryUpdates(self, timeout=None, retries=None):
    """Restarts presentation.device charging so that dumpsys no longer collects power data.
  ...
  def BatteryMeasurement(self, timeout=None, retries=None):
    ...
    the presentation.device appear to stop charging so that dumpsys will start collecting
  ...
  def _DischargeDevice(self, percent, wait_period=120):
    """Disables charging and waits for presentation.device to discharge given amount
  ...
  def ChargeDeviceToLevel(self, level, wait_period=60):
    """Enables charging and waits for presentation.device to be charged to given level.
  ...
  def LetBatteryCoolToTemperature(self, target_temp, wait_period=180):
    """Lets presentation.device sit to give battery time to cool down
    Args:
    ...
    logger.info('Waiting for the presentation.device to cool down to %s (0.1 C)',
                target_temp)
    timeout_retry.WaitFor(cool_device, wait_period=wait_period)

  @decorators.WithTimeoutAndRetriesFromInstance()
  def SetCharging(self, enabled, timeout=None, retries=None):
    """Enables or disables charging on the presentation.device.
  ...
  def _DiscoverDeviceProfile(self):
    """Checks and caches presentation.device information.
