...
class _UiNode(object):
  ...
  def __init__(self, presentation.device, xml_node, package=None):
    ...
    use an AppUi object (below) to grab an xml screenshot from a presentation.device and
    find nodes in it.

    Args:
      presentation.device: A device_utils.DeviceUtils instance.
  ...
  def Tap(self, point=None, dp_units=False):
    ...
    Args:
      ...
      dp_units: If True, indicates that the coordinates of the point are given
        in presentation.device-independent pixels; otherwise they are assumed to be "real"
  ...
  def _Find(self, **kwargs):
    ...
    For example:
      ...
      app = app_ui.AppUi(presentation.device, package='org.my.app')
...
class AppUi(object):
  ...
  def __init__(self, presentation.device, package=None):
