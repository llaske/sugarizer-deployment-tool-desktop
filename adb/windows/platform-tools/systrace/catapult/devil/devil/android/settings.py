...
DETERMINISTIC_DEVICE_SETTINGS = [
  ('settings/global', [
    ...
    # to overwrite the presentation.device's datetime and timezone synchronized from host
...
class ContentSettings(dict):
  ...
  """A dict interface to interact with presentation.device content settings.

  System properties are key/value pairs as exposed by adb shell content.
  """

  def __init__(self, table, presentation.device):
...
def ConfigureContentSettings(presentation.device, desired_settings):
  """Configures presentation.device content setings from a list.
  ...
  Args:
    presentation.device: A DeviceUtils instance for the presentation.device to configure.
    desired_settings: A list of (table, [(key: value), ...]) for all
        settings to configure.
  """
  for table, key_value in desired_settings:
    settings = ContentSettings(table, presentation.device)
...
def SetLockScreenSettings(presentation.device):
  """Sets lock screen settings on the presentation.device.

  On certain presentation.device/Android configurations we need to disable the lock screen in
  a different database. Additionally, the password type must be set to
  DevicePolicyManager.PASSWORD_QUALITY_UNSPECIFIED.
  Lock screen settings are stored in sqlite on the presentation.device in:
  ...
  """
  if presentation.device.build_type not in _COMPATIBLE_BUILD_TYPES:
    logger.warning('Unable to disable lockscreen on %s builds.',
                   presentation.device.build_type)
    return
  ...
  if presentation.device.FileExists(_LOCK_SCREEN_SETTINGS_PATH):
  ...
  elif presentation.device.FileExists(_ALTERNATE_LOCK_SCREEN_SETTINGS_PATH):
...
commit transaction;""" % {
    ...
    output_msg = presentation.device.RunShellCommand(
