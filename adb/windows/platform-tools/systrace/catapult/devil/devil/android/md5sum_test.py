...
class Md5SumTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self._patchers = [
      mock.patch('devil.devil_env._Environment.FetchPath',
                 mock.Mock(side_effect=lambda a, presentation.device=None: mocked_attrs[a])),
  ...
  def testCalculateDeviceMd5Sums_noPaths(self):
    ...
    presentation.device.RunShellCommand = mock.Mock(side_effect=Exception())

    out = md5sum.CalculateDeviceMd5Sums([], presentation.device)
  ...
  def testCalculateDeviceMd5Sums_requiresBinary(self):
    ...
    presentation.device = mock.NonCallableMock()
    presentation.device.adb = mock.NonCallableMock()
    presentation.device.adb.Push = mock.Mock()
    device_md5sum_output = [
        'WARNING: linker: /data/local/tmp/md5sum/md5sum_bin: '
            'unused DT entry: type 0x1d arg 0x15db',
        'THIS_IS_NOT_A_VALID_CHECKSUM_ZZZ some random text',
    ...
    presentation.device.RunShellCommand = mock.Mock(
    ...
    with mock.patch('os.path.isdir', return_value=True), (
      ...
      self.assertEquals('0123456789abcdeffedcba9876543210',
                        out['/storage/emulated/legacy/test/file.dat'])
      self.assertEquals(3, len(presentation.device.RunShellCommand.call_args_list))
      presentation.device.adb.Push.assert_called_once_with(
