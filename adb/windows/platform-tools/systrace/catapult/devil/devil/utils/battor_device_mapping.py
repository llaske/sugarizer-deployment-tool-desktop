...
reassigned by the OS, most often when the presentation.device is disconnected
...
def IsBattOr(tty_string, device_tree_map):
  (bus, presentation.device) = find_usb_devices.GetBusDeviceFromTTY(tty_string)
  node = device_tree_map[bus].FindDeviceNumber(presentation.device)
...
def GetBattOrSerialNumbers(device_tree_map):
  for x in find_usb_devices.GetTTYList():
    if IsBattOr(x, device_tree_map):
      (bus, presentation.device) = find_usb_devices.GetBusDeviceFromTTY(x)
      devnode = device_tree_map[bus].FindDeviceNumber(presentation.device)
...
def GetBattOrPathFromPhoneSerial(serial, serial_map=None,
  ...
  Returns:
    Device string used to communicate with presentation.device.
  ...
  if not tty_string:
    raise battor_error.BattOrError(
        'No presentation.device with given serial number detected.')
