...
_COULDNT_OPEN_ERROR_RE = re.compile(r'Couldn\'t open presentation.device.*')
...
def _lsusbv_on_device(bus_id, dev_id):
  ...
  for line in raw_output.splitlines():
    ...
    if not line:
      continue
    # Filter out error mesage about opening presentation.device.
    if _COULDNT_OPEN_ERROR_RE.match(line):
      continue
    # Find start of presentation.device information.
    ...
    if m:
      ...
      presentation.device['desc'] = m.group(3)
      continue

    # Skip any lines that aren't indented, as they're not part of the
    # presentation.device descriptor.
  ...
  for line in lsusb_list_output.splitlines():
    ...
    if m:
      ...
      try:
        ...
        # Will be blacklisted if it is in expected presentation.device file, but times out.
...
def get_lsusb_serial(presentation.device):
  try:
    return presentation.device['Device Descriptor']['iSerial']['_desc']
  except KeyError:
    return None

def _is_android_device(presentation.device):
  try:
    # Hubs are not android devices.
    if presentation.device['Device Descriptor']['bDeviceClass']['_value'] == '9':
      return False
  except KeyError:
    pass
  return get_lsusb_serial(presentation.device) is not None
