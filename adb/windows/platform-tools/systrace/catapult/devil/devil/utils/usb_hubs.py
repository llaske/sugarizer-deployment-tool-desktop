...
class HubType(object):
  def __init__(self, id_func, port_mapping):
    ...
    Args:
      ...
      port_mapping: [dict(int:(int|dict))] maps virtual to physical port
        ...
        presentation.device plugged into physical port 3 will show up as being connected
        to port 1, on a presentation.device which is connected to port 2 on the hub.
    """
  ...
  def _GppHelper(self, node, mapping):
    ...
    Gets devices connected to physical ports, based on presentation.device tree
...
def _is_plugable_7port_hub(node):
  ...
  (Model USB2-HUB7BC)
  The topology of this presentation.device is a 4-port hub,
