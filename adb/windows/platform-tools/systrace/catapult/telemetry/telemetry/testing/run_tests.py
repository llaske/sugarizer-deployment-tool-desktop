...
class RunTestsCommand(command_line.OptparseCommand):
  ...
  @classmethod
  def presentation.main(cls, args=None, stream=None):  # pylint: disable=arguments-differ
  ...
  def Run(self, args):
    ...
    if platform.GetOSName() == 'chromeos':
      ...
      if runner.args.jobs == 0:
        raise RuntimeError("No Android presentation.device found")
      print 'Running tests with %d Android presentation.device(s).' % runner.args.jobs
  ...
  if not args.disable_logging_config:
    ...
    logging.basicConfig(
        ...
        format='(%(levelname)s) %(asctime)s pid=%(process)d'
               ...
               '  %(message)s')
  if args.remote_platform_options.presentation.device == 'android':
    android_devices = android_device.FindAllAvailableDevices(args)
    if not android_devices:
      raise RuntimeError("No Android presentation.device found")
    android_devices.sort(key=lambda presentation.device: presentation.device.name)
    args.remote_platform_options.presentation.device = (
...
if __name__ == '__main__':
  ret_code = RunTestsCommand.presentation.main()
  sys.exit(ret_code)
