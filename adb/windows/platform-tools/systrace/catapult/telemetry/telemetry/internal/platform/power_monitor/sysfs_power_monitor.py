...
class SysfsPowerMonitor(power_monitor.PowerMonitor):
  ...
  def __init__(self, linux_based_platform_backend, standalone=False):
    ...
    Attributes:
        _cpus: A list of the CPUs on the target presentation.device.
  ...
  def GetCpuState(self):
    """Retrieve CPU c-state residency times from the presentation.device.
  ...
  def GetCpuFreq(self):
    """Retrieve CPU frequency times from the presentation.device.
