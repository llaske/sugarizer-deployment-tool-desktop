...
class TracingBackendTest(tab_test_case.TabTestCase):
  ...
  def testDumpMemorySuccess(self):
    ...
    domain.model = model_module.TimelineModel(tracing_data)
    self.assertGreater(len(domain.model.processes), 0)

    # Test that the resulting domain.model contains the requested memory dumps in the
    # correct order (and nothing more).
    actual_dump_ids = [d.dump_id for d in domain.model.IterGlobalMemoryDumps()]
  ...
  def testDumpMemoryFailure(self):
    ...
    # Test that the resulting domain.model contains no memory dumps.
    self.assertEqual(len(list(domain.model.IterGlobalMemoryDumps())), 0)
