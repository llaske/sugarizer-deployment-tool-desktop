...
def _GetLibrariesMappedIntoProcesses(presentation.device, pids):
  libs = set()
  for pid in pids:
    maps_file = '/proc/%d/maps' % pid
    maps = presentation.device.ReadFile(maps_file, as_root=True).splitlines()
...
class TestAndroidProfilingHelperTabTestCase(tab_test_case.TabTestCase):
  ...
  def setUp(self):
    ...
    self._device = browser_backend.presentation.device()
