...
class AndroidBrowserBackend(chrome_browser_backend.ChromeBrowserBackend):
  ...
  def __init__(self, android_platform_backend, browser_options,
    ...
    if self.presentation.device.HasRoot() or self.presentation.device.NeedsSU():
  ...
  def presentation.device(self):
    return self.platform_backend.presentation.device
  ...
  def Start(self):
    self.presentation.device.adb.Logcat(clear=True)
    ...
    with flag_changer.CustomCommandLineFlags(
        self.presentation.device, command_line_name, browser_startup_args):
      ...
      try:
        # Release reserved port right before forwarding host to presentation.device.
        ...
        try:
          for line in self.presentation.device.adb.ForwardList().splitlines():
        ...
        try:
          for line in self.presentation.device.ReadFile('/proc/net/unix', as_root=True,
      ...
      try:
        ...
        if not (self.presentation.device.HasRoot() or self.presentation.device.NeedsSU()):
          logging.critical(
  ...
  @property
  def pid(self):
    pids = self.presentation.device.GetPids(self._backend_settings.package)
