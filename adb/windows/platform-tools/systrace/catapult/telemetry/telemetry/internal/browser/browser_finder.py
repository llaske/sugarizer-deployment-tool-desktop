  ...
  for presentation.device in devices:
    for finder in BROWSER_FINDERS:
      if(options.browser_type and options.browser_type != 'any' and
        ...
        continue
      curr_browsers = finder.FindAllAvailableBrowsers(options, presentation.device)
...
@decorators.Cache
def GetAllAvailableBrowsers(options, presentation.device):
  """Returns a list of available browsers on the presentation.device.

  Args:
    options: A BrowserOptions object.
    presentation.device: The target presentation.device, which can be None.
  ...
  """
  if not presentation.device:
    return []
  possible_browsers = []
  for browser_finder in BROWSER_FINDERS:
    possible_browsers.extend(
      browser_finder.FindAllAvailableBrowsers(options, presentation.device))
...
def GetAllAvailableBrowserTypes(options):
  ...
  possible_browsers = []
  for presentation.device in devices:
    possible_browsers.extend(GetAllAvailableBrowsers(options, presentation.device))
