...
class _SingleProcessPerfProfiler(object):
  ...
  def __init__(self, pid, output_file, browser_backend, platform_backend,
    ...
    if self._is_android:
      cmd_prefix = [
          browser_backend.presentation.device.adb.GetAdbPath(),
          '-s', browser_backend.presentation.device.adb.GetDeviceSerial(),
          'shell', perf_binary]
      perf_args += _PERF_OPTIONS_ANDROID
      output_file = os.path.join('/sdcard', 'perf_profiles',
                                 os.path.basename(output_file))
      self._device_output_file = output_file
      browser_backend.presentation.device.RunShellCommand(
          ['mkdir', '-p', os.path.dirname(self._device_output_file)],
          check_return=True)
      browser_backend.presentation.device.RemovePath(self._device_output_file, force=True)
    else:
  ...
  def CollectProfile(self):
    ...
    if self._is_android:
      presentation.device = self._browser_backend.presentation.device
      try:
        binary_name = os.path.basename(self._perf_binary)
        presentation.device.KillAll(binary_name, signum=signal.SIGINT, blocking=True,
                       quiet=True)
      except device_errors.CommandFailedError:
        logging.warning('The perf process could not be killed on the presentation.device.')
...
/proc/sys/kernel/perf_event_paranoid to "-1".\nOutput:\n%s""" %
    ...
    if self._is_android:
      presentation.device = self._browser_backend.presentation.device
      presentation.device.PullFile(self._device_output_file, self._output_file)
      required_libs = \
      ...
      kallsyms = android_profiling_helper.CreateSymFs(presentation.device,
                                                      symfs_root,
                                                      required_libs,
                                                      use_symlinks=True)
  ...
  def __init__(self, browser_backend, platform_backend, output_path, state):
    ...
    try:
      if platform_backend.GetOSName() == 'android':
        presentation.device = browser_backend.presentation.device
        perf_binary = android_profiling_helper.PrepareDeviceForPerf(presentation.device)
        self._perf_control = perf_control.PerfControl(presentation.device)
