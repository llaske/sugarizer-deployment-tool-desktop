...
# actually talking to the presentation.device. This would improve our coverage quite
# a bit.
...
class CrOSInterfaceTest(unittest.TestCase):
  ...
  def testGetRemotePortAndIsHTTPServerRunningOnPort(self):
    with self._GetCRI() as cri:
      ...
      # Forward local server's port to remote presentation.device's remote_port.
      forwarder = cros_forwarder.CrOsForwarderFactory(cri).Create(
          forwarders.PortPairs(http=forwarders.PortPair(port, remote_port),
                               https=None,
                               dns=None))

      # At this point, remote presentation.device should be able to connect to local server.
  ...
  def testEscapeCmdArguments(self):
    ...
    and locally on the presentation.device to check for consistency.
    """
  ...
  def testIsCryptohomeMounted(self, mock_run_cmd):
    # The presentation.device's mount state is checked by the command
