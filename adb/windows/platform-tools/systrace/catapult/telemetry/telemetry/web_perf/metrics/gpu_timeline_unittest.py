...
from telemetry.timeline import domain.model as model_module
...
class GPUTimelineTest(unittest.TestCase):
  def GetResults(self, metric, domain.model, renderer_thread, interaction_records):
    results = test_page_test_results.TestPageTestResults(self)
    metric.AddResults(domain.model, renderer_thread, interaction_records, results)
    return results

  def testExpectedResults(self):
    """Test a simply trace will output all expected results."""
    domain.model = model_module.TimelineModel()
    test_thread = domain.model.GetOrCreateProcess(1).GetOrCreateThread(2)
    for slice_item in _CreateGPUSlices(test_thread, 'test_item', 100, 10):
      _AddSliceToThread(test_thread, slice_item)
    domain.model.FinalizeImport()

    metric = gpu_timeline.GPUTimelineMetric()
    results = self.GetResults(metric, domain.model=domain.model, renderer_thread=test_thread,
  ...
  def testNoDeviceTraceResults(self):
    """Test expected results when missing presentation.device traces."""
    domain.model = model_module.TimelineModel()
    test_thread = domain.model.GetOrCreateProcess(1).GetOrCreateThread(2)
  ...
  def testOutOfOrderDeviceTraces(self):
    """Out of order presentation.device traces are still matched up to correct services."""
