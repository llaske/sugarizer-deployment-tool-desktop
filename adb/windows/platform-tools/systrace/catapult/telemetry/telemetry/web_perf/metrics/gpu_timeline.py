...
TOPLEVEL_DEVICE_CATEGORY = 'disabled-by-default-gpu.presentation.device'
...
class GPUTimelineMetric(timeline_based_metric.TimelineBasedMetric):
  ...
  def _CalculateGPUTimelineData(self, domain.model):
    """Uses the domain.model and calculates the times for various values for each
       ...
       Source Types:
         ...
         gpu - For an event, the "gpu" source type signifies time spent on the
               gpu thread using the GPU. This uses the "gpu.presentation.device" markers.
    """
    all_service_events = []
    ...
    event_iter = domain.model.IterAllEvents(
    ...
    # Add in GPU side traces if it was supported (IE. presentation.device traces exist).
