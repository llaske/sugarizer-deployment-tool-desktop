...
class TimelineBasedMeasurement(story_test.StoryTest):
  ...
  def _ComputeLegacyTimelineBasedMetrics(self, results, trace_result):
    domain.model = model_module.TimelineModel(trace_result)
    threads_to_records_map = _GetRendererThreadsToInteractionRecordsMap(domain.model)
    ...
    for renderer_thread, interaction_records in (
      ...
      meta_metrics = _TimelineBasedMetrics(
          domain.model, renderer_thread, interaction_records, self._results_wrapper,
          all_metrics)
      meta_metrics.AddResults(results)

    for metric in all_metrics:
      metric.AddWholeTraceResults(domain.model, results)
