...
class FakeFilesystem(object):
  ...
  def RemoveObject(self, file_path):
    ...
    """
    if file_path == self.root.name:
      raise OSError(errno.EBUSY, 'Fake presentation.device or resource busy',
                    file_path)
    try:
...
class FakeOsModule(object):
  ...
  def mknod(self, filename, mode=None, presentation.device=None):
    """Create a filesystem node named 'filename'.

    Does not support presentation.device special files or named pipes as the real os
    module does.
    ...
    Args:
      ...
      mode: (int) permissions to use and type of file to be created.
        ...
        to this mode.
      presentation.device: not supported in fake implementation
    ...
    if presentation.device or not mode & stat.S_IFREG:
