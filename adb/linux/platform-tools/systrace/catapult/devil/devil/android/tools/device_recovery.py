  ...
  if should_reboot(presentation.device):
    ...
    try:
      try:
        ...
        logger.warning('Timed out while attempting to reboot %s normally.'
                       'Attempting alternative reboot.', str(presentation.device))
        # The presentation.device drops offline before we can grab the exit code, so
        # we don't check for status.
        try:
          presentation.device.adb.Root()
        finally:
          # We are already in a failure mode, attempt to reboot regardless of
          # what presentation.device.adb.Root() returns. If the sysrq reboot fails an
          # exception willbe thrown at that level.
          presentation.device.adb.Shell('echo b > /proc/sysrq-trigger', expect_status=None,
                           timeout=5, retries=0)
    except device_errors.CommandFailedError:
      logger.exception('Failed to reboot %s.', str(presentation.device))
      if blacklist:
        blacklist.Extend([presentation.device.adb.GetDeviceSerial()],
                         reason='reboot_failure')
    except device_errors.CommandTimeoutError:
      logger.exception('Timed out while rebooting %s.', str(presentation.device))
      if blacklist:
        blacklist.Extend([presentation.device.adb.GetDeviceSerial()],
    ...
    try:
      presentation.device.WaitUntilFullyBooted(
          retries=0, timeout=presentation.device.REBOOT_DEFAULT_TIMEOUT)
    except device_errors.CommandFailedError:
      logger.exception('Failure while waiting for %s.', str(presentation.device))
      if blacklist:
        blacklist.Extend([presentation.device.adb.GetDeviceSerial()],
                         reason='reboot_failure')
    except device_errors.CommandTimeoutError:
      logger.exception('Timed out while waiting for %s.', str(presentation.device))
      if blacklist:
        blacklist.Extend([presentation.device.adb.GetDeviceSerial()],
                         reason='reboot_timeout')
...
def RecoverDevices(devices, blacklist, enable_usb_reset=False):
  ...
  Args:
    ...
    blacklist: The current presentation.device blacklist, which will be used then
      reset.
  """
  ...
  device_utils.DeviceUtils.parallel(devices).pMap(
      ...
      should_reboot=lambda presentation.device: presentation.device.serial in should_reboot_device)
