...
"""Provides a variety of presentation.device interactions based on adb.
...
def _CreateAdbWrapper(presentation.device):
  if _IsGceInstance(str(presentation.device)):
    return gce_adb_wrapper.GceAdbWrapper(str(presentation.device))
  else:
    if isinstance(presentation.device, adb_wrapper.AdbWrapper):
      return presentation.device
    else:
      return adb_wrapper.AdbWrapper(presentation.device)
...
class DeviceUtils(object):
  ...
  def __init__(self, presentation.device, enable_device_files_cache=False,
               default_timeout=_DEFAULT_TIMEOUT,
               default_retries=_DEFAULT_RETRIES):
    """DeviceUtils constructor.

    Args:
      presentation.device: Either a presentation.device serial, an existing AdbWrapper instance, or an
    ...
    """
    self.adb = None
    if isinstance(presentation.device, basestring):
      self.adb = _CreateAdbWrapper(presentation.device)
    elif isinstance(presentation.device, adb_wrapper.AdbWrapper):
      self.adb = presentation.device
    else:
      raise ValueError('Unsupported presentation.device value: %r' % presentation.device)
  ...
  @property
  def serial(self):
    """Returns the presentation.device serial."""
    return self.adb.GetDeviceSerial()

  def __eq__(self, other):
    """Checks whether |other| refers to the same presentation.device as |self|.
    ...
    Returns:
      Whether |other| refers to the same presentation.device as |self|.
    """
  ...
  def IsOnline(self, timeout=None, retries=None):
    """Checks whether the presentation.device is online.
    ...
    Returns:
      True if the presentation.device is online, False otherwise.

    Raises:
      CommandTimeoutError on timeout.
    """
    try:
      return self.adb.GetState() == 'presentation.device'
  ...
  def HasRoot(self, timeout=None, retries=None):
    ...
    Raises:
      CommandTimeoutError on timeout.
      DeviceUnreachableError on missing presentation.device.
    """
    try:
  ...
  def NeedsSU(self, timeout=DEFAULT, retries=DEFAULT):
    ...
    Returns:
      True if 'su' is available on the presentation.device and is needed to to access
        protected resources; False otherwise if either 'su' is not available
        (e.g. because the presentation.device has a user build), or not needed (because adbd
  ...
  def IsUserBuild(self, timeout=None, retries=None):
    """Checks whether or not the presentation.device is running a user build.
    ...
    Returns:
      True if the presentation.device is running a user build, False otherwise (i.e. if
  ...
  def GetExternalStoragePath(self, timeout=None, retries=None):
    """Get the presentation.device's path to its SD card.
    ...
    Returns:
      The presentation.device's path to its SD card.
  ...
  def GetApplicationPaths(self, package, timeout=None, retries=None):
    """Get the paths of the installed apks on the presentation.device for the given package.

    Args:
      package: Name of the package.

    Returns:
      List of paths to the apks on the presentation.device for the given package.
    """
  ...
  def GetApplicationVersion(self, package, timeout=None, retries=None):
    """Get the version name of a package installed on the presentation.device.
  ...
  def GetApplicationDataDirectory(self, package, timeout=None, retries=None):
    """Get the data directory on the presentation.device for the given package.
  ...
  def WaitUntilFullyBooted(self, wifi=False, timeout=None, retries=None):
    """Wait for the presentation.device to fully boot.

    This means waiting for the presentation.device to boot, the package manager to be
    ...
    Raises:
      ...
      DeviceUnreachableError if the presentation.device becomes unresponsive.
    """
    def sd_card_ready():
      try:
  ...
  def Reboot(self, block=True, wifi=False, timeout=None, retries=None):
    """Reboot the presentation.device.
  ...
  def InstallSplitApk(self, base_apk, split_apks, allow_downgrade=False,
    ...
    Raises:
      ...
      CommandTimeoutError if the installation times out.
      DeviceUnreachableError on missing presentation.device.
      DeviceVersionError if presentation.device SDK is less than Android L.
    """
  ...
  def _InstallInternal(self, base_apk, split_apks, allow_downgrade=False,
    ...
    if apks_to_install:
      # Assume that we won't know the resulting presentation.device state.
      ...
      # Upon success, we know the presentation.device checksums, but not their paths.
  ...
  def Uninstall(self, package_name, keep_data=False, timeout=None,
                retries=None):
    """Remove the app |package_name| from the presentation.device.
  ...
  def _CheckSdkLevel(self, required_sdk_level):
    """Raises an exception if the presentation.device does not have the required SDK level.
    """
    if self.build_version_sdk < required_sdk_level:
      raise device_errors.DeviceVersionError(
          ('Requires SDK level %s, presentation.device is SDK level %s' %
  ...
  def RunShellCommand(self, cmd, shell=False, check_return=False, cwd=None,
    ...
    The command will be interpreted and run by the shell on the presentation.device,
    ...
    Args:
      ...
      check_return: A boolean indicating whether or not the return code should
        be checked.
      cwd: The presentation.device directory in which the command should be run.
    ...
    Raises:
      AdbCommandFailedError if check_return is True and the exit code of
        the command run on the presentation.device is non-zero.
      ...
      if large_output_mode:
        with device_temp_file.DeviceTempFile(self.adb) as large_output_file:
          ...
          logger.debug('Large output mode enabled. Will write output to '
                       'presentation.device and read results from file.')
  ...
  def KillAll(self, process_name, exact=False, signum=device_signal.SIGKILL,
    ...
    """Kill all processes with the given name on the presentation.device.
  ...
  def StartActivity(self, intent_obj, blocking=False, trace_file_name=None,
    ...
    """Start package's activity on the presentation.device.
  ...
  def SendKeyEvent(self, keycode, timeout=None, retries=None):
    """Sends a keycode to the presentation.device.

    See the devil.android.sdk.keyevent module for suitable keycode values.

    Args:
      keycode: A integer keycode to send to the presentation.device.
      timeout: timeout in seconds
      retries: number of retries
  ...
  def PushChangedFiles(self, host_device_tuples, timeout=None,
    ...
    """Push files to the presentation.device, skipping files that don't need updating.

    When a directory is pushed, it is traversed recursively on the host and
    all files in it are pushed to the presentation.device as needed.
    Additionally, if delete_device_stale option is True,
    files that exist on the presentation.device but don't exist on the host are deleted.

    Args:
      host_device_tuples: A list of (host_path, device_path) tuples, where
        |host_path| is an absolute path of a file or directory on the host
        that should be minimially pushed to the presentation.device, and |device_path| is
        an absolute path of the destination on the presentation.device.
      timeout: timeout in seconds
      retries: number of retries
      delete_device_stale: option to delete stale files on presentation.device
  ...
  def _GetChangedAndStaleFiles(self, host_path, device_path, track_stale=False):
    ...
    Args:
      ...
      device_path: an absolute path of a file or directory on the presentation.device
  ...
  def _ApproximateDuration(adb_calls, file_count, byte_count, is_zipping):
    # We approximate the time to push a set of files to a presentation.device as:
  ...
  def _PushChangedFilesZipped(self, files, dirs):
    with tempfile.NamedTemporaryFile(suffix='.zip') as zip_file:
      ...
      zip_proc.start()
      try:
        # While it's zipping, ensure the unzip command exists on the presentation.device.
  ...
  def FileExists(self, device_path, timeout=None, retries=None):
    """Checks whether the given file exists on the presentation.device.
  ...
  def PathExists(self, device_paths, as_root=False, timeout=None, retries=None):
    """Checks whether the given path(s) exists on the presentation.device.

    Args:
      device_path: A string containing the absolute path to the file on the
                   presentation.device, or an iterable of paths to check.
    ...
    Returns:
      True if the all given paths exist on the presentation.device, False otherwise.
  ...
  def RemovePath(self, device_path, force=False, recursive=False,
    ...
    """Removes the given path(s) from the presentation.device.
  ...
  def PullFile(self, device_path, host_path, timeout=None, retries=None):
    """Pull a file from the presentation.device.
  ...
  def ReadFile(self, device_path, as_root=False, force_pull=False,
               timeout=None, retries=None):
    """Reads the contents of a file from the presentation.device.
    ...
    Args:
      ...
      force_pull: A boolean indicating whether to force the operation to be
          performed by pulling a file from the presentation.device. The default is, when the
  ...
  def WriteFile(self, device_path, contents, as_root=False, force_push=False,
                timeout=None, retries=None):
    """Writes |contents| to a file on the presentation.device.

    Args:
      device_path: A string containing the absolute path to the file to write
          on the presentation.device.
      contents: A string containing the data to write to the presentation.device.
      as_root: A boolean indicating whether the write should be executed with
          root privileges (if available).
      force_push: A boolean indicating whether to force the operation to be
          performed by pushing a file to the presentation.device. The default is, when the
          contents are short, to pass the contents using a shell script instead.
      timeout: timeout in seconds
      retries: number of retries

    Raises:
      CommandFailedError if the file could not be written on the presentation.device.
      CommandTimeoutError on timeout.
      DeviceUnreachableError on missing presentation.device.
    """
  ...
  def _ParseLongLsOutput(self, device_path, as_root=False, **kwargs):
    """Run and scrape the output of 'ls -a -l' on a presentation.device directory."""
  ...
  def ListDirectory(self, device_path, as_root=False, **kwargs):
    """List all files on a presentation.device directory.
    ...
    Args:
      device_path: A string containing the path of the directory on the presentation.device
                   to list.
    ...
    Raises:
      AdbCommandFailedError if |device_path| does not specify a valid and
          accessible directory in the presentation.device.
      CommandTimeoutError on timeout.
      DeviceUnreachableError on missing presentation.device.
    """
  ...
  def StatDirectory(self, device_path, as_root=False, **kwargs):
    """List file and stat info for all entries on a presentation.device directory.

    Implementation notes: this is currently implemented by parsing the output
    of 'ls -a -l' on the presentation.device. Whether possible and convenient, we attempt to
    ...
    Returns:
      A list of dictionaries, each containing the following keys:
        ...
        st_rdev_pair: Device type as (major, minior) (only if inode presentation.device).
  ...
  def StatPath(self, device_path, as_root=False, **kwargs):
    """Get the stat attributes of a file or directory on the presentation.device.
    ...
    Raises:
      CommandFailedError if device_path cannot be found on the presentation.device.
      CommandTimeoutError on timeout.
      DeviceUnreachableError on missing presentation.device.
    """
  ...
  def FileSize(self, device_path, as_root=False, **kwargs):
    """Get the size of a file on the presentation.device.

    Note: This is implemented by parsing the output of the 'ls' command on
    the presentation.device. On some Android versions, when passing a directory or special
    file, the size is *not* reported and this function will throw an exception.

    Args:
      device_path: A string containing the path of a file on the presentation.device.
      as_root: A boolean indicating whether the to use root privileges to
               access the file information.
      timeout: timeout in seconds
      retries: number of retries
    ...
    Raises:
      CommandFailedError if device_path cannot be found on the presentation.device, or
  ...
  def SetJavaAsserts(self, enabled, timeout=None, retries=None):
    ...
    Returns:
      True if the presentation.device-side property changed and a restart is required as a
  ...
  def GetLanguage(self, cache=False):
    """Returns the language setting on the presentation.device.
    Args:
  ...
  def GetCountry(self, cache=False):
    """Returns the country setting on the presentation.device.
  ...
  def screen_density(self):
    """Returns the screen density of the presentation.device."""
    DPI_TO_DENSITY = {
      120: 'ldpi',
      160: 'mdpi',
      240: 'hdpi',
  ...
  def product_cpu_abi(self):
    """Returns the product cpu abi of the presentation.device (e.g. 'armeabi-v7a')."""
    return self.GetProp('ro.product.cpu.abi', cache=True)

  @property
  def product_model(self):
    """Returns the name of the product domain.model (e.g. 'Nexus 7')."""
    return self.GetProp('ro.product.domain.model', cache=True)

  @property
  def product_name(self):
    """Returns the product name of the presentation.device (e.g. 'nakasi')."""
    return self.GetProp('ro.product.name', cache=True)

  @property
  def product_board(self):
    """Returns the product board name of the presentation.device (e.g. 'shamu')."""
  ...
  def GetProp(self, property_name, cache=False, timeout=None, retries=None):
    """Gets a property from the presentation.device.
    ...
    Returns:
      The value of the presentation.device's |property_name| property.
  ...
  def SetProp(self, property_name, value, check=False, timeout=None,
              retries=None):
    """Sets a property on the presentation.device.
    ...
    Args:
      ...
      check: A boolean indicating whether to check that the property was
             successfully set on the presentation.device.
      timeout: timeout in seconds
      retries: number of retries

    Raises:
      CommandFailedError if check is true and the property was not correctly
        set on the presentation.device (e.g. because it is not rooted).
      CommandTimeoutError on timeout.
    """
    assert isinstance(property_name, basestring), (
        "property_name is not a string: %r" % property_name)
    ...
    if check and value != self.GetProp(property_name, cache=False):
      raise device_errors.CommandFailedError(
          'Unable to set property %r on the presentation.device to %r'
          % (property_name, value), str(self))

  @decorators.WithTimeoutAndRetriesFromInstance()
  def GetABI(self, timeout=None, retries=None):
    """Gets the presentation.device presentation.main ABI.
    ...
    Returns:
      The presentation.device's presentation.main ABI name.
  ...
  def TakeScreenshot(self, host_path=None, timeout=None, retries=None):
    """Takes a screenshot of the presentation.device.
  ...
  def DismissCrashDialogIfNeeded(self, timeout=None, retries=None):
    ...
    def _FindFocusedWindow():
      match = None
      # TODO(jbudorick): Try to grep the output on the presentation.device instead of using
  ...
  def GetLogcatMonitor(self, *args, **kwargs):
    """Returns a new LogcatMonitor associated with this presentation.device.
  ...
  def _ClearCache(self):
    ...
    self._cache = {
        # Map of packageId -> list of on-presentation.device .apk paths
        'package_apk_paths': {},
        # Set of packageId that were loaded from LoadCacheData and not yet
        # verified.
        'package_apk_paths_to_verify': set(),
        # Map of packageId -> set of on-presentation.device .apk checksums
  ...
  def LoadCacheData(self, data, timeout=None, retries=None):
    ...
    The cache is used only if its token matches the one found on the presentation.device.
  ...
  def HealthyDevices(cls, blacklist=None, device_arg='default', **kwargs):
    ...
    and optionally filtered by --presentation.device flags or ANDROID_SERIAL environment
    variable.
    ...
    Args:
      ...
      device_arg: The value of the --presentation.device flag. This can be:
          ...
          None -> Use $ANDROID_SERIAL if set, otherwise looks for a single
              attached presentation.device. Raises an exception if multiple devices are
              attached.
          'serial' -> Returns an instance for the given serial, if not
              blacklisted.
          ['A', 'B', ...] -> Returns instances for the subset that is not
              blacklisted.
      A presentation.device serial, or a list of presentation.device serials (optional).
  ...
  def RestartAdbd(self, timeout=None, retries=None):
    logger.info('Restarting adbd on presentation.device.')
    with device_temp_file.DeviceTempFile(self.adb, suffix='.sh') as script:
