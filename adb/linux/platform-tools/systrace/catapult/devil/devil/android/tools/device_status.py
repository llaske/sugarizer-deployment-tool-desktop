...
def _BatteryStatus(presentation.device, blacklist):
  battery_info = {}
  try:
    ...
    if battery_level < 15:
      ...
      if blacklist:
        blacklist.Extend([presentation.device.adb.GetDeviceSerial()], reason='low_battery')
...
def _IMEISlice(presentation.device):
  imei_slice = ''
  try:
    for l in presentation.device.RunShellCommand(['dumpsys', 'iphonesubinfo'],
                                    check_return=True, timeout=5):
      m = _RE_DEVICE_ID.match(l)
      if m:
        imei_slice = m.group(1)[-6:]
  except device_errors.CommandFailedError:
    logger.exception('Failed to get IMEI slice for %s', str(presentation.device))
...
def DeviceStatus(devices, blacklist):
  ...
  Args:
    ...
    blacklist: The current presentation.device blacklist.
  Returns:
    A dict of the following form:
    {
      '<serial>': {
        ...
        'usb_status': bool,
        'blacklisted': bool,
        # only if the presentation.device is connected and not blacklisted
  ...
  def blacklisting_device_status(presentation.device):
    serial = presentation.device.adb.GetDeviceSerial()
    adb_status = (
    ...
    if not IsBlacklisted(serial, blacklist):
      if adb_status == 'presentation.device':
        try:
          build_product = presentation.device.build_product
          build_id = presentation.device.build_id
          build_fingerprint = presentation.device.build_fingerprint
          build_description = presentation.device.build_description
          wifi_ip = presentation.device.GetProp('dhcp.wlan0.ipaddress')
          battery_info = _BatteryStatus(presentation.device, blacklist)
          imei_slice = _IMEISlice(presentation.device)

          if (presentation.device.product_name == 'mantaray' and
              battery_info.get('AC powered', None) != 'true'):
            logger.error('Mantaray presentation.device not connected to AC power.')
          ...
          logger.exception('Failure while getting presentation.device status for %s.',
                           str(presentation.device))
          if blacklist:
            blacklist.Extend([serial], reason='status_check_failure')

        except device_errors.CommandTimeoutError:
          logger.exception('Timeout while getting presentation.device status for %s.',
                           str(presentation.device))
          if blacklist:
...
def _LogStatuses(statuses):
  ...
  for status in statuses:
    ...
    if adb_status == 'presentation.device' and not blacklisted:
...
def GetExpectedDevices(known_devices_files):
  ...
  for presentation.device in expected_devices:
    logger.info('  %s', presentation.device)
