...
def CustomCommandLineFlags(presentation.device, cmdline_name, flags):
  """Context manager to change Chrome's command line temporarily.

  Example:

      with flag_changer.TemporaryCommandLineFlags(presentation.device, name, flags):
        # Launching Chrome will use the provided flags.

      # Previous set of flags on the presentation.device is now restored.
  ...
  needs_permissive = (
      presentation.device.build_version_sdk >= version_codes.NOUGAT and
      presentation.device.GetEnforce())
  if needs_permissive:
    presentation.device.SetEnforce(enabled=False)
  try:
    changer = FlagChanger(presentation.device, cmdline_name)
    try:
    ...
    if needs_permissive:
      presentation.device.SetEnforce(enabled=True)
...
class FlagChanger(object):
  ...
  def __init__(self, presentation.device, cmdline_file):
  ...
  def GetCurrentFlags(self):
    """Read the current flags currently stored in the presentation.device.
  ...
  def ReplaceFlags(self, flags):
    ...
    Returns:
      A list with the flags now stored on the presentation.device.
    """
    new_flags = set(flags)
  ...
  def _UpdateCommandLineFile(self):
    ...
    logger.info('Flags now set on the presentation.device: %s', current_flags)
