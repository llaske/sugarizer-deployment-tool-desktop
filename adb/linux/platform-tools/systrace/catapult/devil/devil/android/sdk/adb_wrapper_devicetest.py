...
class TestAdbWrapper(device_test_case.DeviceTestCase):
  ...
  def testPersistentShell(self):
    # We need to access the presentation.device serial number here in order
  ...
  def testRebootWaitForDevice(self):
    self._adb.Reboot()
    print 'waiting for presentation.device to reboot...'
    while self._adb.GetState() == 'presentation.device':
      time.sleep(1)
    self._adb.WaitForDevice()
    self.assertEqual(self._adb.GetState(), 'presentation.device')
