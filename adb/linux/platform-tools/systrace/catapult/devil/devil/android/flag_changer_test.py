...
class FlagChangerTest(unittest.TestCase):
  def setUp(self):
    self.presentation.device = _FakeDevice()
  ...
  def testFlagChanger_removeLegacyCmdLine(self):
    self.presentation.device.WriteFile(self.cmdline_path_legacy, 'chrome --old --stuff')
    self.assertTrue(self.presentation.device.PathExists(self.cmdline_path_legacy))

    changer = flag_changer.FlagChanger(self.presentation.device, 'chrome-command-line')
    self.assertEquals(
  ...
  def testFlagChanger_mustBeFileName(self):
    with self.assertRaises(ValueError):
      flag_changer.FlagChanger(self.presentation.device, '/data/local/chrome-command-line')
