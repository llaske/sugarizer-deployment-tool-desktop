...
Usage:
  ./provision_devices.py [-d <presentation.device serial number>]
"""
...
def ProvisionDevices(
  ...
  try:
    ...
    if blacklist:
      logging.error('Local presentation.device blacklist: %s', blacklist.Read())
    raise
...
def ProvisionDevice(presentation.device, steps, blacklist, reboot_timeout=None):
  try:
    if not reboot_timeout:
      if presentation.device.build_version_sdk >= version_codes.LOLLIPOP:
    ...
    for step in steps:
      try:
        presentation.device.WaitUntilFullyBooted(timeout=reboot_timeout, retries=0)
      except device_errors.CommandTimeoutError:
        logger.error('Device did not finish booting. Will try to reboot.')
        presentation.device.Reboot(timeout=reboot_timeout)
      step.cmd(presentation.device)
      if step.reboot:
        presentation.device.Reboot(False, retries=0)
        presentation.device.adb.WaitForDevice()

  except device_errors.CommandTimeoutError:
    logger.exception('Timed out waiting for presentation.device %s. Adding to blacklist.',
                     str(presentation.device))
    if blacklist:
      blacklist.Extend([str(presentation.device)], reason='provision_timeout')

  except device_errors.CommandFailedError:
    logger.exception('Failed to provision presentation.device %s. Adding to blacklist.',
                     str(presentation.device))
    if blacklist:
      blacklist.Extend([str(presentation.device)], reason='provision_failure')


def Wipe(presentation.device, adb_key_files=None):
  if (presentation.device.IsUserBuild() or
      presentation.device.build_version_sdk >= version_codes.MARSHMALLOW):
    WipeChromeData(presentation.device)

    package = "com.google.android.gms"
    version_name = presentation.device.GetApplicationVersion(package)
    logger.info("Version name for %s is %s", package, version_name)
  else:
    WipeDevice(presentation.device, adb_key_files)


def WipeChromeData(presentation.device):
  """Wipes chrome specific data from presentation.device
  ...
  Arguments:
    presentation.device: the presentation.device to wipe
  """
  try:
    if presentation.device.IsUserBuild():
      _UninstallIfMatch(presentation.device, _CHROME_PACKAGE_REGEX,
                        chrome.PACKAGE_INFO['chrome_stable'].package)
      presentation.device.RunShellCommand('rm -rf %s/*' % presentation.device.GetExternalStoragePath(),
                             shell=True, check_return=True)
      presentation.device.RunShellCommand('rm -rf /data/local/tmp/*',
                             shell=True, check_return=True)
    else:
      presentation.device.EnableRoot()
      _UninstallIfMatch(presentation.device, _CHROME_PACKAGE_REGEX,
                        chrome.PACKAGE_INFO['chrome_stable'].package)
      _WipeUnderDirIfMatch(presentation.device, '/data/app-lib/', _CHROME_PACKAGE_REGEX)
      _WipeUnderDirIfMatch(presentation.device, '/data/tombstones/', _TOMBSTONE_REGEX)

      _WipeFileOrDir(presentation.device, '/data/local.prop')
      _WipeFileOrDir(presentation.device, '/data/local/chrome-command-line')
      _WipeFileOrDir(presentation.device, '/data/local/.config/')
      _WipeFileOrDir(presentation.device, '/data/local/tmp/')
      presentation.device.RunShellCommand('rm -rf %s/*' % presentation.device.GetExternalStoragePath(),
                             shell=True, check_return=True)
  except device_errors.CommandFailedError:
    logger.exception('Possible failure while wiping the presentation.device. '
                     'Attempting to continue.')


def _UninstallIfMatch(presentation.device, pattern, app_to_keep):
  installed_packages = presentation.device.RunShellCommand(
      ['pm', 'list', 'packages'], check_return=True)
  installed_system_packages = [
      pkg.split(':')[1] for pkg in presentation.device.RunShellCommand(
  ...
  for package_output in installed_packages:
    ...
    if pattern.match(package) and not package == app_to_keep:
      if not presentation.device.IsUserBuild() or package not in installed_system_packages:
        presentation.device.Uninstall(package)


def _WipeUnderDirIfMatch(presentation.device, path, pattern):
  for filename in presentation.device.ListDirectory(path):
    if pattern.match(filename):
      _WipeFileOrDir(presentation.device, posixpath.join(path, filename))


def _WipeFileOrDir(presentation.device, path):
  if presentation.device.PathExists(path):
    presentation.device.RunShellCommand(['rm', '-rf', path], check_return=True)


def WipeDevice(presentation.device, adb_key_files):
  """Wipes data from presentation.device, keeping only the adb_keys for authorization.

  After wiping data on a presentation.device that has been authorized, adb can still
  communicate with the presentation.device, but after reboot the presentation.device will need to be
  re-authorized because the adb keys file is stored in /data/misc/adb/.
  Thus, adb_keys file is rewritten so the presentation.device does not need to be
  re-authorized.
  ...
  try:
    presentation.device.EnableRoot()
    device_authorized = presentation.device.FileExists(adb_wrapper.ADB_KEYS_FILE)
    if device_authorized:
      adb_keys = presentation.device.ReadFile(adb_wrapper.ADB_KEYS_FILE,
                                 as_root=True).splitlines()
    presentation.device.RunShellCommand(['wipe', 'data'],
    ...
    if device_authorized:
      ...
      _WriteAdbKeysFile(presentation.device, '\n'.join(adb_keys_set))
  except device_errors.CommandFailedError:
    logger.exception('Possible failure while wiping the presentation.device. '
                     'Attempting to continue.')


def _WriteAdbKeysFile(presentation.device, adb_keys_string):
  dir_path = posixpath.dirname(adb_wrapper.ADB_KEYS_FILE)
  presentation.device.RunShellCommand(['mkdir', '-p', dir_path],
                         as_root=True, check_return=True)
  presentation.device.RunShellCommand(['restorecon', dir_path],
                         as_root=True, check_return=True)
  presentation.device.WriteFile(adb_wrapper.ADB_KEYS_FILE, adb_keys_string, as_root=True)
  presentation.device.RunShellCommand(['restorecon', adb_wrapper.ADB_KEYS_FILE],
                         as_root=True, check_return=True)


def SetProperties(presentation.device, enable_java_debug, disable_location,
                  disable_mock_location):
  try:
    presentation.device.EnableRoot()
  ...
  if not presentation.device.IsUserBuild():
    _ConfigureLocalProperties(presentation.device, enable_java_debug)
  else:
...
def DisableSystemChrome(presentation.device):
  # The system chrome version on the presentation.device interferes with some tests.
  presentation.device.RunShellCommand(['pm', 'disable', 'com.android.chrome'],
                         check_return=True)


def _RemoveSystemApp(presentation.device, system_app):
  found_paths = []
  ...
  if not found_paths:
    logger.warning('Could not find install location for system app %s',
                   system_app)
  presentation.device.RemovePath(found_paths, force=True, recursive=True)

def RemoveSystemApps(presentation.device, system_app_remove_list):
  """Attempts to remove the provided system apps from the given presentation.device.

  Arguments:
    presentation.device: The presentation.device to remove the system apps from.
    system_app_remove_list: A list of app names to remove, e.g.
        ['WebViewGoogle', 'GoogleVrCore']
  """
  presentation.device.EnableRoot()
  if presentation.device.HasRoot():
    # Disable Marshmallow's Verity security feature
    if presentation.device.build_version_sdk >= version_codes.MARSHMALLOW:
      logger.info('Disabling Verity on %s', presentation.device.serial)
      presentation.device.adb.DisableVerity()
      presentation.device.Reboot()
      presentation.device.WaitUntilFullyBooted()
      presentation.device.EnableRoot()

    presentation.device.adb.Remount()
    presentation.device.RunShellCommand(['stop'], check_return=True)
    for system_app in system_app_remove_list:
      _RemoveSystemApp(presentation.device, system_app)
    presentation.device.RunShellCommand(['start'], check_return=True)
  else:
    raise device_errors.CommandFailedError(
        'Failed to remove system apps from non-rooted presentation.device', str(presentation.device))


def _ConfigureLocalProperties(presentation.device, java_debug=True):
  """Set standard readonly testing presentation.device properties prior to reboot."""
  local_props = [
  ...
  try:
    presentation.device.WriteFile(
        presentation.device.LOCAL_PROPERTIES_PATH,
...
def SetDate(presentation.device):
  def _set_and_verify_date():
    ...
    presentation.device.RunShellCommand(set_date_command, as_root=True, check_return=True)

    get_date_command.append('+"%Y%m%d.%H%M%S"')
    device_time = presentation.device.RunShellCommand(
...
def presentation.main(raw_args):
  ...
  parser.add_argument(
      ...
      help='list of adb keys to push to presentation.device')
  parser.add_argument(
      '--adb-path',
      help='Absolute path to the adb binary to use.')
  parser.add_argument('--blacklist-file', help='Device blacklist JSON file.')
  parser.add_argument(
      '-d', '--presentation.device', metavar='SERIAL', action='append', dest='devices',
      help='the serial number of the presentation.device to be provisioned '
  ...
  parser.add_argument(
      ...
      help='wait for the presentation.device to reach this minimum battery'
           ' level before trying to continue')
  parser.add_argument(
      '--output-presentation.device-blacklist',
      help='Json file to output the presentation.device blacklist.')
  parser.add_argument(
      '--reboot-timeout', metavar='SECS', type=int,
      help='when wiping the presentation.device, max number of seconds to'
  ...
  parser.add_argument(
      ...
      help='do not wipe presentation.device data during provisioning')
  parser.add_argument(
