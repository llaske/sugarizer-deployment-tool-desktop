...
"""Takes a screenshot from an Android presentation.device."""
...
def presentation.main():
  ...
  def screenshot(presentation.device):
    f = None
    if host_file:
      root, ext = os.path.splitext(host_file)
      f = '%s_%s%s' % (root, str(presentation.device), ext)
    f = presentation.device.TakeScreenshot(f)
    print 'Screenshot for presentation.device %s written to %s' % (
        str(presentation.device), os.path.abspath(f))
