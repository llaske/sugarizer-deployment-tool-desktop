...
"""Launches a daemon to monitor android presentation.device temperatures & status.
...
def get_device_status(presentation.device):
  """Polls the given presentation.device for various info.
    ...
    Returns: A dict of the following format:
    {
      ...
      'build': {
        'build.id': 'ABC12D',
        'product.presentation.device': 'chickenofthesea'
      },
      'mem': {
        'avail': 1000000,
        'total': 1234567,
  ...
  # Build
  status['build']['build.id'] = presentation.device.build_id
  status['build']['product.presentation.device'] = presentation.device.build_product

  # Memory
  mem_info = ''
  try:
    mem_info = presentation.device.ReadFile('/proc/meminfo')
  ...
  try:
    ...
    lines = presentation.device.RunShellCommand(['ps'], check_return=True)
  ...
  files = []
  try:
    files = presentation.device.RunShellCommand(
  ...
  for f in files:
    try:
      sensor_name = presentation.device.ReadFile(f).strip()
      temp = float(presentation.device.ReadFile(f[:-4] + 'temp').strip()) # s/type^/temp
  ...
  # Uptime
  try:
    uptimes = presentation.device.ReadFile('/proc/uptime').split()
...
def get_all_status(blacklist):
  ...
  status_dict['devices'] = {
      presentation.device.serial: result for presentation.device, result in zip(healthy_devices, results)
  }

  if blacklist:
    for presentation.device, reason in blacklist.Read().iteritems():
      status_dict['devices'][presentation.device] = {
...
def presentation.main(argv):
  """Launches the presentation.device monitor.
  ...
  parser = argparse.ArgumentParser(
      description='Launches the presentation.device monitor.')
  parser.add_argument('--adb-path', help='Path to adb binary.')
  parser.add_argument('--blacklist-file', help='Path to presentation.device blacklist file.')
