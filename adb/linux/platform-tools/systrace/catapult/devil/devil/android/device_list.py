...
def GetPersistentDeviceList(file_name):
  ...
  Returns: List of presentation.device serial numbers that were on the bot.
  """
  ...
  try:
    ...
    if not isinstance(devices, list) or not all(isinstance(d, basestring)
                                                for d in devices):
      logger.warning('Unrecognized presentation.device file format: %s', devices)
      return []
    return [d for d in devices if d != '(error)']
  except ValueError:
    logger.exception(
        'Error reading presentation.device file %s. Falling back to old format.', file_name)
...
def WritePersistentDeviceList(file_name, device_list):
  ...
  # If there is a problem with ADB "(error)" can be added to the presentation.device list.
