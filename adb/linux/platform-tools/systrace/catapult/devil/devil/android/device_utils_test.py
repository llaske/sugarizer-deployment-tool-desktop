...
class DeviceUtilsTest(mock_calls.TestCase):
  ...
  def setUp(self):
    ...
    self.presentation.device = device_utils.DeviceUtils(
  ...
  def EnsureCacheInitialized(self, props=None, sdcard='/sdcard'):
    ...
    return (self.call.presentation.device.RunShellCommand(
...
class DeviceUtilsEqTest(DeviceUtilsTest):
  ...
  def testEq_equal_deviceUtils(self):
    ...
    self.assertTrue(self.presentation.device == other)
    self.assertTrue(other == self.presentation.device)
  ...
  def testEq_devicesNotEqual(self):
    ...
    self.assertFalse(self.presentation.device == other)
    self.assertFalse(other == self.presentation.device)

  def testEq_identity(self):
    self.assertTrue(self.presentation.device == self.presentation.device)

  def testEq_serialInList(self):
    devices = [self.presentation.device]
...
class DeviceUtilsStrTest(DeviceUtilsTest):
  ...
  def testStr_returnsSerial(self):
    with self.assertCalls(
      ...
      self.assertEqual('0123456789abcdef', str(self.presentation.device))


class DeviceUtilsIsOnlineTest(DeviceUtilsTest):

  def testIsOnline_true(self):
    with self.assertCall(self.call.adb.GetState(), 'presentation.device'):
      self.assertTrue(self.presentation.device.IsOnline())

  def testIsOnline_false(self):
    with self.assertCall(self.call.adb.GetState(), 'offline'):
      self.assertFalse(self.presentation.device.IsOnline())
...
class DeviceUtilsHasRootTest(DeviceUtilsTest):
  ...
  def testHasRoot_true(self):
    with self.patch_call(self.call.presentation.device.product_name,
                          return_value='notasailfish'), (
        self.assertCall(self.call.adb.Shell('ls /root'), 'foo\n')):
      self.assertTrue(self.presentation.device.HasRoot())
  ...
  def testHasRoot_false(self):
    with self.patch_call(self.call.presentation.device.product_name,
        ...
        self.assertCall(self.call.adb.Shell('ls /root'),
                        self.ShellError())):
      self.assertFalse(self.presentation.device.HasRoot())
...
class DeviceUtilsEnableRootTest(DeviceUtilsTest):
  ...
  def testEnableRoot_succeeds(self):
    with self.assertCalls(
        (self.call.presentation.device.IsUserBuild(), False),
         self.call.adb.Root(),
         self.call.presentation.device.WaitUntilFullyBooted()):
      self.presentation.device.EnableRoot()

  def testEnableRoot_userBuild(self):
    with self.assertCalls(
        (self.call.presentation.device.IsUserBuild(), True)):
...
class DeviceUtilsIsUserBuildTest(DeviceUtilsTest):
  ...
  def testIsUserBuild_yes(self):
    with self.assertCall(
        self.call.presentation.device.GetProp('ro.build.type', cache=True), 'user'):
      self.assertTrue(self.presentation.device.IsUserBuild())

  def testIsUserBuild_no(self):
    with self.assertCall(
        self.call.presentation.device.GetProp('ro.build.type', cache=True), 'userdebug'):
      self.assertFalse(self.presentation.device.IsUserBuild())
  ...
  def testGetExternalStoragePath_succeeds(self):
    with self.assertCalls(
      ...
      self.assertEquals('/fake/storage/path',
                        self.presentation.device.GetExternalStoragePath())
...
class DeviceUtilsGetApplicationPathsInternalTest(DeviceUtilsTest):
  ...
  def testGetApplicationPathsInternal_exists(self):
    with self.assertCalls(
        (self.call.presentation.device.GetProp('ro.build.version.sdk', cache=True), '19'),
        (self.call.presentation.device.RunShellCommand(
            ['pm', 'path', 'android'], check_return=True),
         ['package:/path/to/android.apk'])):
      self.assertEquals(['/path/to/android.apk'],
                        self.presentation.device._GetApplicationPathsInternal('android'))
  ...
  def testGetApplicationPathsInternal_notExists(self):
    with self.assertCalls(
        ...
        (self.call.presentation.device.RunShellCommand(
         ...
         '')):
      self.assertEquals([],
          self.presentation.device._GetApplicationPathsInternal('not.installed.app'))
  ...
  def test_GetApplicationVersion_exists(self):
    with self.assertCalls(
      ...
      self.assertEquals('45.0.1234.7',
                        self.presentation.device.GetApplicationVersion('com.android.chrome'))
...
class DeviceUtilsGetApplicationDataDirectoryTest(DeviceUtilsTest):
  ...
  def testGetApplicationDataDirectory_exists(self):
    with self.assertCall(
        self.call.presentation.device._RunPipedShellCommand(
            'pm dump foo.bar.baz | grep dataDir='),
        ['dataDir=/data/data/foo.bar.baz']):
      self.assertEquals(
          '/data/data/foo.bar.baz',
          self.presentation.device.GetApplicationDataDirectory('foo.bar.baz'))
...
class DeviceUtilsWaitUntilFullyBootedTest(DeviceUtilsTest):
  ...
  def testWaitUntilFullyBooted_succeedsNoWifi(self):
    with self.assertCalls(
        ...
        # sd_card_ready
        (self.call.presentation.device.GetExternalStoragePath(), '/fake/storage/path'),
        (self.call.adb.Shell('test -d /fake/storage/path'), ''),
        # pm_ready
        (self.call.presentation.device._GetApplicationPathsInternal('android',
                                                       skip_cache=True),
         ['package:/some/fake/path']),
        # boot_completed
        (self.call.presentation.device.GetProp('sys.boot_completed', cache=False), '1')):
      self.presentation.device.WaitUntilFullyBooted(wifi=False)
  ...
  def testWaitUntilFullyBooted_succeedsWithWifi(self):
    with self.assertCalls(
        ...
        (self.call.adb.Shell('dumpsys wifi'),
         'stuff\nWi-Fi is enabled\nmore stuff\n')):
      self.presentation.device.WaitUntilFullyBooted(wifi=True)

  def testWaitUntilFullyBooted_deviceNotInitiallyAvailable(self):
    with self.assertCalls(
        self.call.adb.WaitForDevice(),
        # sd_card_ready
        (self.call.presentation.device.GetExternalStoragePath(), self.AdbCommandError()),
        # sd_card_ready
        (self.call.presentation.device.GetExternalStoragePath(), self.AdbCommandError()),
        # sd_card_ready
        (self.call.presentation.device.GetExternalStoragePath(), self.AdbCommandError()),
  ...
  def testWaitUntilFullyBooted_deviceBrieflyOffline(self):
    with self.assertCalls(
        ...
        (self.call.presentation.device._GetApplicationPathsInternal('android',
         ...
         ['package:/some/fake/path']),
        # boot_completed
        (self.call.presentation.device.GetProp('sys.boot_completed', cache=False),
  ...
  def testWaitUntilFullyBooted_sdCardReadyFails_noPath(self):
    with self.assertCalls(
        self.call.adb.WaitForDevice(),
        # sd_card_ready
        (self.call.presentation.device.GetExternalStoragePath(), self.CommandError())):
      with self.assertRaises(device_errors.CommandFailedError):
        self.presentation.device.WaitUntilFullyBooted(wifi=False)
  ...
  def testWaitUntilFullyBooted_bootFails(self):
    with self.assertCalls(
        ...
        (self.call.presentation.device._GetApplicationPathsInternal('android',
         ...
         ['package:/some/fake/path']),
        # boot_completed
        (self.call.presentation.device.GetProp('sys.boot_completed', cache=False), '0'),
        # boot_completed
        (self.call.presentation.device.GetProp('sys.boot_completed', cache=False), '0'),
        # boot_completed
        (self.call.presentation.device.GetProp('sys.boot_completed', cache=False),
...
class DeviceUtilsRebootTest(DeviceUtilsTest):
  ...
  def testReboot_nonBlocking(self):
    with self.assertCalls(
        ...
        (self.call.presentation.device.IsOnline(), True),
        (self.call.presentation.device.IsOnline(), False)):
      self.presentation.device.Reboot(block=False)
  ...
  def testReboot_blocking(self):
    with self.assertCalls(
        ...
        (self.call.presentation.device.IsOnline(), True),
        (self.call.presentation.device.IsOnline(), False),
        self.call.presentation.device.WaitUntilFullyBooted(wifi=False)):
      self.presentation.device.Reboot(block=True)
  ...
  def testReboot_blockUntilWifi(self):
    with self.assertCalls(
        ...
        (self.call.presentation.device.IsOnline(), True),
        (self.call.presentation.device.IsOnline(), False),
        self.call.presentation.device.WaitUntilFullyBooted(wifi=True)):
      self.presentation.device.Reboot(block=True, wifi=True)
...
class DeviceUtilsInstallTest(DeviceUtilsTest):
  ...
  def testInstall_noPriorInstall(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk, return_value=23):
      with self.assertCalls(
          (mock.call.os.path.exists('/fake/test/app.apk'), True),
          (self.call.presentation.device._GetApplicationPathsInternal('test.package'), []),
          self.call.adb.Install('/fake/test/app.apk', reinstall=False,
                                allow_downgrade=False),
          (self.call.presentation.device.GrantPermissions('test.package', ['p1']), [])):
        self.presentation.device.Install(DeviceUtilsInstallTest.mock_apk, retries=0)

  def testInstall_permissionsPreM(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk, return_value=20):
      with self.assertCalls(
          (mock.call.os.path.exists('/fake/test/app.apk'), True),
          (self.call.presentation.device._GetApplicationPathsInternal('test.package'), []),
          (self.call.adb.Install('/fake/test/app.apk', reinstall=False,
  ...
  def testInstall_passPermissions(self):
    with self.assertCalls(
        ...
        (self.call.adb.Install('/fake/test/app.apk', reinstall=False,
                               allow_downgrade=False)),
        (self.call.presentation.device.GrantPermissions('test.package', ['p1', 'p2']), [])):
      self.presentation.device.Install(DeviceUtilsInstallTest.mock_apk, retries=0,
                          permissions=['p1', 'p2'])
  ...
  def testInstall_differentPriorInstall(self):
    with self.assertCalls(
        ...
        (self.call.presentation.device._ComputeStaleApks('test.package',
            ['/fake/test/app.apk']),
         (['/fake/test/app.apk'], None)),
        self.call.presentation.device.Uninstall('test.package'),
        self.call.adb.Install('/fake/test/app.apk', reinstall=False,
                              allow_downgrade=False)):
      self.presentation.device.Install(DeviceUtilsInstallTest.mock_apk, retries=0,
                          permissions=[])
  ...
  def testInstall_differentPriorInstall_reinstall(self):
    with self.assertCalls(
        ...
        self.call.adb.Install('/fake/test/app.apk', reinstall=True,
                              allow_downgrade=False)):
      self.presentation.device.Install(DeviceUtilsInstallTest.mock_apk,
  ...
  def testInstall_identicalPriorInstall_reinstall(self):
    with self.assertCalls(
        ...
        (self.call.presentation.device._ComputeStaleApks('test.package',
            ['/fake/test/app.apk']),
         ([], None)),
        (self.call.presentation.device.ForceStop('test.package'))):
      self.presentation.device.Install(DeviceUtilsInstallTest.mock_apk,
          reinstall=True, retries=0, permissions=[])
...
class DeviceUtilsInstallSplitApkTest(DeviceUtilsTest):
  ...
  def testInstallSplitApk_noPriorInstall(self):
    with self.assertCalls(
        (self.call.presentation.device._CheckSdkLevel(21)),
        (mock.call.devil.android.sdk.split_select.SelectSplits(
            self.presentation.device, 'base.apk',
        ...
        (self.call.adb.InstallMultiple(
            ...
            allow_downgrade=False))):
      self.presentation.device.InstallSplitApk(DeviceUtilsInstallSplitApkTest.mock_apk,
  ...
  def testInstallSplitApk_partialInstall(self):
    with self.assertCalls(
        ...
        (mock.call.os.path.exists('split2.apk'), True),
        (self.call.presentation.device._GetApplicationPathsInternal('test.package'),
         ['base-on-presentation.device.apk', 'split2-on-presentation.device.apk']),
        (self.call.presentation.device._ComputeStaleApks('test.package',
                                            ['base.apk', 'split2.apk']),
         (['split2.apk'], None)),
        (self.call.adb.InstallMultiple(
...
class DeviceUtilsUninstallTest(DeviceUtilsTest):
  ...
  def testUninstall_callsThrough(self):
    with self.assertCalls(
      ...
      self.presentation.device.Uninstall('test.package', True)
...
class DeviceUtilsSuTest(DeviceUtilsTest):
  ...
  def testSu_preM(self):
    with self.patch_call(
        self.call.presentation.device.build_version_sdk,
        return_value=version_codes.LOLLIPOP_MR1):
      self.assertEquals('su -c foo', self.presentation.device._Su('foo'))

  def testSu_mAndAbove(self):
    with self.patch_call(
        self.call.presentation.device.build_version_sdk,
        return_value=version_codes.MARSHMALLOW):
      self.assertEquals('su 0 foo', self.presentation.device._Su('foo'))
...
class DeviceUtilsRunShellCommandTest(DeviceUtilsTest):
  ...
  def setUp(self):
    ...
    self.presentation.device.NeedsSU = mock.Mock(return_value=False)

  def testRunShellCommand_commandAsList(self):
    with self.assertCall(self.call.adb.Shell('pm list packages'), ''):
      self.presentation.device.RunShellCommand(
  ...
  def testRunShellCommand_withHugeCmd(self):
    ...
    with self.assertCalls(
      ...
      self.call.presentation.device._WriteFileWithPush('/sdcard/temp-123.sh', expected_cmd),
      (self.call.adb.Shell('sh /sdcard/temp-123.sh'), payload + '\n')):
      self.assertEquals(
          [payload],
          self.presentation.device.RunShellCommand(['echo', payload], check_return=True))
  ...
  def testRunShellCommand_withHugeCmdAndSu(self):
    ...
    with self.assertCalls(
      (self.call.presentation.device.NeedsSU(), True),
      (self.call.presentation.device._Su(expected_cmd_without_su), expected_cmd),
      (mock.call.devil.android.device_temp_file.DeviceTempFile(
          self.adb, suffix='.sh'), MockTempFile('/sdcard/temp-123.sh')),
      self.call.presentation.device._WriteFileWithPush('/sdcard/temp-123.sh', expected_cmd),
      (self.call.adb.Shell('sh /sdcard/temp-123.sh'), payload + '\n')):
  ...
  def testRunShellCommand_manyLines(self):
    ...
    with self.assertCall(self.call.adb.Shell(cmd), 'file1\nfile2\nfile3\n'):
      self.assertEquals(
          ['file1', 'file2', 'file3'],
          self.presentation.device.RunShellCommand(cmd.split(), check_return=True))
  ...
  def testRunShellCommand_checkReturn_success(self):
    ...
    with self.assertCall(self.call.adb.Shell(cmd), output):
      self.assertEquals(
          [output.rstrip()],
          self.presentation.device.RunShellCommand(cmd, shell=True, check_return=True))
  ...
  def testRunShellCommand_checkReturn_disabled(self):
    ...
    with self.assertCall(self.call.adb.Shell(cmd), self.ShellError(output)):
      self.assertEquals(
          [output.rstrip()],
          self.presentation.device.RunShellCommand(cmd.split(), check_return=False))
  ...
  def testRunShellCommand_largeOutput_enabled(self):
    ...
    with self.assertCalls(
        ...
        (self.call.presentation.device.ReadFile(temp_file.name, force_pull=True),
         'something')):
      self.assertEquals(
          ['something'],
          self.presentation.device.RunShellCommand(
  ...
  def testRunShellCommand_largeOutput_disabledNoTrigger(self):
    ...
    with self.assertCall(self.call.adb.Shell(cmd), self.ShellError('')):
      with self.assertRaises(device_errors.AdbCommandFailedError):
        self.presentation.device.RunShellCommand([cmd], check_return=True)
  ...
  def testRunShellCommand_largeOutput_disabledTrigger(self):
    ...
    with self.assertCalls(
        ...
        (mock.call.devil.android.device_temp_file.DeviceTempFile(self.adb),
            temp_file),
        (self.call.adb.Shell(cmd_redirect)),
        (self.call.presentation.device.ReadFile(mock.ANY, force_pull=True),
         'something')):
      self.assertEquals(
          ['something'],
          self.presentation.device.RunShellCommand(cmd, shell=True, check_return=True))
  ...
  def testRunPipedShellCommand_success(self):
    with self.assertCall(
      ...
      self.assertEquals(['This line contains foo'],
                        self.presentation.device._RunPipedShellCommand('ps | grep foo'))
...
class DeviceUtilsKillAllTest(DeviceUtilsTest):
  ...
  def testKillAll_noMatchingProcessesFailure(self):
    with self.assertCall(self.call.presentation.device.GetPids('test_process'), {}):
      with self.assertRaises(device_errors.CommandFailedError):
        self.presentation.device.KillAll('test_process')

  def testKillAll_noMatchingProcessesQuiet(self):
    with self.assertCall(self.call.presentation.device.GetPids('test_process'), {}):
      self.assertEqual(0, self.presentation.device.KillAll('test_process', quiet=True))

  def testKillAll_nonblocking(self):
    with self.assertCalls(
        (self.call.presentation.device.GetPids('some.process'),
         {'some.process': ['1234'], 'some.processing.thing': ['5678']}),
        (self.call.adb.Shell('kill -9 1234 5678'), '')):
      self.assertEquals(
          2, self.presentation.device.KillAll('some.process', blocking=False))
  ...
  def testKillAll_blocking(self):
    with self.assertCalls(
      ...
      self.assertEquals(
          2, self.presentation.device.KillAll('some.process', blocking=True))
  ...
  def testKillAll_exactNonblocking(self):
    with self.assertCalls(
      ...
      self.assertEquals(
          1, self.presentation.device.KillAll('some.process', exact=True, blocking=False))
  ...
  def testKillAll_exactBlocking(self):
    with self.assertCalls(
        ...
        (self.call.presentation.device.GetPids('some.process'),
         {'some.processing.thing': ['5678']})):
      self.assertEquals(
          1, self.presentation.device.KillAll('some.process', exact=True, blocking=True))

  def testKillAll_root(self):
    with self.assertCalls(
        (self.call.presentation.device.GetPids('some.process'), {'some.process': ['1234']}),
        (self.call.presentation.device.NeedsSU(), True),
        (self.call.presentation.device._Su("sh -c 'kill -9 1234'"),
         "su -c sh -c 'kill -9 1234'"),
        (self.call.adb.Shell("su -c sh -c 'kill -9 1234'"), '')):
      self.assertEquals(
          1, self.presentation.device.KillAll('some.process', as_root=True))
  ...
  def testKillAll_sigterm(self):
    with self.assertCalls(
      ...
      self.assertEquals(
          1, self.presentation.device.KillAll('some.process', signum=device_signal.SIGTERM))
  ...
  def testKillAll_multipleInstances(self):
    with self.assertCalls(
      ...
      self.assertEquals(
          2, self.presentation.device.KillAll('some.process', signum=device_signal.SIGTERM))
...
class DeviceUtilsStartActivityTest(DeviceUtilsTest):
  ...
  def testStartActivity_actionOnly(self):
    ...
    with self.assertCall(
      ...
      self.presentation.device.StartActivity(test_intent)
  ...
  def testStartActivity_blocking(self):
    ...
    with self.assertCall(
        self.call.adb.Shell('am start '
                            ...
                            '-n test.package/.Main'),
        'Starting: Intent { act=android.intent.action.VIEW }'):
      self.presentation.device.StartActivity(test_intent, blocking=True)
  ...
  def testStartActivity_withForceStop(self):
    ...
    with self.assertCall(
        self.call.adb.Shell('am start '
                            ...
                            '-n test.package/.Main'),
        'Starting: Intent { act=android.intent.action.VIEW }'):
      self.presentation.device.StartActivity(test_intent, force_stop=True)
...
class DeviceUtilsStartInstrumentationTest(DeviceUtilsTest):
  ...
  def testStartInstrumentation_nothing(self):
    with self.assertCalls(
      ...
      self.presentation.device.StartInstrumentation(
  ...
  def testStartInstrumentation_finish(self):
    with self.assertCalls(
        (self.call.presentation.device.RunShellCommand(
            ...
            shell=True, check_return=True, large_output=True),
         ['OK (1 test)'])):
      output = self.presentation.device.StartInstrumentation(
          'test.package/.TestInstrumentation',
...
class DeviceUtilsBroadcastIntentTest(DeviceUtilsTest):
  ...
  def testBroadcastIntent_noExtras(self):
    ...
    with self.assertCall(
      ...
      self.presentation.device.BroadcastIntent(test_intent)
...
class DeviceUtilsGoHomeTest(DeviceUtilsTest):
  ...
  def testGoHome_popupsExist(self):
    with self.assertCalls(
      ...
      self.presentation.device.GoHome()
  ...
  def testForceStop(self):
    with self.assertCall(
        self.call.adb.Shell('p=test.package;if [[ "$(ps)" = *$p* ]]; then '
                            'am force-stop $p; fi'),
        ''):
      self.presentation.device.ForceStop('test.package')
...
class DeviceUtilsClearApplicationStateTest(DeviceUtilsTest):
  ...
  def testClearApplicationState_setPermissions(self):
    with self.assertCalls(
        (self.call.presentation.device.GetProp('ro.build.version.sdk', cache=True), '17'),
        (self.call.presentation.device._GetApplicationPathsInternal('this.package.exists'),
         ['/data/app/this.package.exists.apk']),
        (self.call.presentation.device.RunShellCommand(
            ['pm', 'clear', 'this.package.exists'],
            check_return=True),
         ['Success']),
        (self.call.presentation.device.GrantPermissions(
            'this.package.exists', ['p1']), [])):
      self.presentation.device.ClearApplicationState(
          'this.package.exists', permissions=['p1'])

  def testClearApplicationState_packageDoesntExist(self):
    with self.assertCalls(
        (self.call.presentation.device.GetProp('ro.build.version.sdk', cache=True), '11'),
        (self.call.presentation.device._GetApplicationPathsInternal('does.not.exist'),
         [])):
      self.presentation.device.ClearApplicationState('does.not.exist')

  def testClearApplicationState_packageDoesntExistOnAndroidJBMR2OrAbove(self):
    with self.assertCalls(
        (self.call.presentation.device.GetProp('ro.build.version.sdk', cache=True), '18'),
        (self.call.presentation.device.RunShellCommand(
            ['pm', 'clear', 'this.package.does.not.exist'],
            check_return=True),
         ['Failed'])):
      self.presentation.device.ClearApplicationState('this.package.does.not.exist')
  ...
  def testClearApplicationState_packageExists(self):
    with self.assertCalls(
        ...
        (self.call.presentation.device.RunShellCommand(
            ...
            check_return=True),
         ['Success'])):
      self.presentation.device.ClearApplicationState('this.package.exists')
...
class DeviceUtilsSendKeyEventTest(DeviceUtilsTest):
  ...
  def testSendKeyEvent(self):
    with self.assertCall(self.call.adb.Shell('input keyevent 66'), ''):
      self.presentation.device.SendKeyEvent(66)
...
class DeviceUtilsPushChangedFilesIndividuallyTest(DeviceUtilsTest):
  ...
  def testPushChangedFilesIndividually_empty(self):
    test_files = []
    with self.assertCalls():
      self.presentation.device._PushChangedFilesIndividually(test_files)

  def testPushChangedFilesIndividually_single(self):
    test_files = [('/test/host/path', '/test/presentation.device/path')]
    with self.assertCalls(self.call.adb.Push(*test_files[0])):
      self.presentation.device._PushChangedFilesIndividually(test_files)

  def testPushChangedFilesIndividually_multiple(self):
    test_files = [
        ('/test/host/path/file1', '/test/presentation.device/path/file1'),
        ('/test/host/path/file2', '/test/presentation.device/path/file2')]
    with self.assertCalls(
...
class DeviceUtilsPushChangedFilesZippedTest(DeviceUtilsTest):
  ...
  def testPushChangedFilesZipped_noUnzipCommand(self):
    test_files = [('/test/host/path/file1', '/test/presentation.device/path/file1')]
    ...
    with self.assertCalls(
        ...
        (self.call.presentation.device._MaybeInstallCommands(), False)):
      self.assertFalse(self.presentation.device._PushChangedFilesZipped(test_files,
                                                           ['/test/dir']))
  ...
  def _testPushChangedFilesZipped_spec(self, test_files):
    ...
    with self.assertCalls(
        ...
        (mock.call.multiprocessing.Process(
            target=device_utils.DeviceUtils._CreateDeviceZip,
            args=('/test/temp/file/tmp.zip', test_files)), mock.Mock()),
        (self.call.presentation.device._MaybeInstallCommands(), True),
        (self.call.presentation.device.NeedsSU(), True),
        (mock.call.devil.android.device_temp_file.DeviceTempFile(self.adb,
                                                                 suffix='.zip'),
        ...
        self.call.presentation.device.RunShellCommand(
            ...
            check_return=True)):
      self.assertTrue(self.presentation.device._PushChangedFilesZipped(test_files,
                                                          ['/test/dir']))
...
class DeviceUtilsPathExistsTest(DeviceUtilsTest):
  ...
  def testPathExists_pathExists(self):
    with self.assertCall(
        ...
        []):
      self.assertTrue(self.presentation.device.PathExists('/path/file exists'))
  ...
  def testPathExists_multiplePathExists(self):
    with self.assertCall(
        self.call.presentation.device.RunShellCommand(
            ...
            as_root=False, check_return=True, timeout=10, retries=0),
        []):
      self.assertTrue(self.presentation.device.PathExists(('/path 1', '/path2')))
  ...
  def testPathExists_pathDoesntExist(self):
    with self.assertCall(
        self.call.presentation.device.RunShellCommand(
            ...
            as_root=False, check_return=True, timeout=10, retries=0),
        self.ShellError()):
      self.assertFalse(self.presentation.device.PathExists('/path/file.not.exists'))
  ...
  def testPathExists_asRoot(self):
    with self.assertCall(
        ...
        self.ShellError()):
      self.assertFalse(
          self.presentation.device.PathExists('/root/path/exists', as_root=True))
  ...
  def testFileExists_pathDoesntExist(self):
    with self.assertCall(
        self.call.presentation.device.RunShellCommand(
            ...
            as_root=False, check_return=True, timeout=10, retries=0),
        self.ShellError()):
      self.assertFalse(self.presentation.device.FileExists('/path/file.not.exists'))
...
class DeviceUtilsRemovePathTest(DeviceUtilsTest):
  ...
  def testRemovePath_regular(self):
    with self.assertCall(
        ...
        []):
      self.presentation.device.RemovePath('some file')
  ...
  def testRemovePath_withForce(self):
    with self.assertCall(
        ...
        []):
      self.presentation.device.RemovePath('some file', force=True)
  ...
  def testRemovePath_recursively(self):
    with self.assertCall(
        ...
        []):
      self.presentation.device.RemovePath('/remove/this/dir', recursive=True)
  ...
  def testRemovePath_withRoot(self):
    with self.assertCall(
        ...
        []):
      self.presentation.device.RemovePath('some file', as_root=True)
  ...
  def testRemovePath_manyPaths(self):
    with self.assertCall(
        ...
        []):
      self.presentation.device.RemovePath(['eeny', 'meeny', 'miny', 'moe'])
    ...
    with mock.patch('os.path.exists', return_value=True):
      with self.assertCall(
          self.call.adb.Pull('/data/app/test.file.exists',
                             '/test/file/host/path')):
        self.presentation.device.PullFile('/data/app/test.file.exists',
                             '/test/file/host/path')
  ...
  def testPullFile_doesntExistOnDevice(self):
    with mock.patch('os.path.exists', return_value=True):
      with self.assertCall(
        ...
        with self.assertRaises(device_errors.CommandFailedError):
          self.presentation.device.PullFile('/data/app/test.file.does.not.exist',
                               '/test/file/host/path')
...
class DeviceUtilsReadFileTest(DeviceUtilsTest):
  ...
  def testReadFileWithPull_success(self):
    ...
    with self.assertCalls(
        ...
        (self.call.adb.Pull('/path/to/presentation.device/file', mock.ANY)),
      ...
      self.assertEquals('some interesting contents',
                        self.presentation.device._ReadFileWithPull('/path/to/presentation.device/file'))
  ...
  def testReadFile_exists(self):
    with self.assertCalls(
        (self.call.presentation.device.FileSize('/read/this/test/file', as_root=False), 256),
        (self.call.presentation.device.RunShellCommand(
            ['cat', '/read/this/test/file'],
            as_root=False, check_return=True),
         ['this is a test file'])):
      self.assertEqual('this is a test file\n',
                       self.presentation.device.ReadFile('/read/this/test/file'))
  ...
  def testReadFile_doesNotExist(self):
    with self.assertCall(
        self.call.presentation.device.FileSize('/this/file/does.not.exist', as_root=False),
        self.CommandError('File does not exist')):
      with self.assertRaises(device_errors.CommandFailedError):
        self.presentation.device.ReadFile('/this/file/does.not.exist')

  def testReadFile_zeroSize(self):
    with self.assertCalls(
        (self.call.presentation.device.FileSize('/this/file/has/zero/size', as_root=False),
         0),
        (self.call.presentation.device._ReadFileWithPull('/this/file/has/zero/size'),
         'but it has contents\n')):
      self.assertEqual('but it has contents\n',
                       self.presentation.device.ReadFile('/this/file/has/zero/size'))

  def testReadFile_withSU(self):
    with self.assertCalls(
        (self.call.presentation.device.FileSize(
      ...
      self.assertEqual(
          'this is a test file\nread with su\n',
          self.presentation.device.ReadFile('/this/file/can.be.read.with.su',
                               as_root=True))

  def testReadFile_withPull(self):
    contents = 'a' * 123456
    with self.assertCalls(
        (self.call.presentation.device.FileSize('/read/this/big/test/file', as_root=False),
         123456),
        (self.call.presentation.device._ReadFileWithPull('/read/this/big/test/file'),
         contents)):
      self.assertEqual(
          contents, self.presentation.device.ReadFile('/read/this/big/test/file'))
  ...
  def testReadFile_withPullAndSU(self):
    ...
    with self.assertCalls(
        ...
        (self.call.presentation.device.NeedsSU(), True),
        (mock.call.devil.android.device_temp_file.DeviceTempFile(self.adb),
         MockTempFile('/sdcard/tmp/on.presentation.device')),
        self.call.presentation.device.RunShellCommand(
            'SRC=/this/big/file/can.be.read.with.su DEST=/sdcard/tmp/on.presentation.device;'
            'cp "$SRC" "$DEST" && chmod 666 "$DEST"',
            shell=True, as_root=True, check_return=True),
        (self.call.presentation.device._ReadFileWithPull('/sdcard/tmp/on.presentation.device'),
         contents)):
      self.assertEqual(
          contents,
          self.presentation.device.ReadFile('/this/big/file/can.be.read.with.su',
                               as_root=True))
  ...
  def testReadFile_forcePull(self):
    ...
    with self.assertCall(
      ...
      self.assertEqual(
          contents,
          self.presentation.device.ReadFile('/read/this/big/test/file', force_pull=True))
...
class DeviceUtilsWriteFileTest(DeviceUtilsTest):
  ...
  def testWriteFileWithPush_success(self):
    ...
    with self.assertCalls(
        ...
        self.call.adb.Push('/tmp/file/on.host', '/path/to/presentation.device/file')):
      self.presentation.device._WriteFileWithPush('/path/to/presentation.device/file', contents)
  ...
  def testWriteFile_withPush(self):
    ...
    with self.assertCalls(
        self.call.presentation.device._WriteFileWithPush('/path/to/presentation.device/file', contents)):
      self.presentation.device.WriteFile('/path/to/presentation.device/file', contents)

  def testWriteFile_withPushForced(self):
    contents = 'tiny contents'
    with self.assertCalls(
        self.call.presentation.device._WriteFileWithPush('/path/to/presentation.device/file', contents)):
      self.presentation.device.WriteFile('/path/to/presentation.device/file', contents, force_push=True)
  ...
  def testWriteFile_withPushAndSU(self):
    ...
    with self.assertCalls(
        ...
        (mock.call.devil.android.device_temp_file.DeviceTempFile(self.adb),
         MockTempFile('/sdcard/tmp/on.presentation.device')),
        self.call.presentation.device._WriteFileWithPush('/sdcard/tmp/on.presentation.device', contents),
        self.call.presentation.device.RunShellCommand(
            ['cp', '/sdcard/tmp/on.presentation.device', '/path/to/presentation.device/file'],
            as_root=True, check_return=True)):
      self.presentation.device.WriteFile('/path/to/presentation.device/file', contents, as_root=True)

  def testWriteFile_withEcho(self):
    with self.assertCall(self.call.adb.Shell(
        "echo -n the.contents > /test/file/to.write"), ''):
      self.presentation.device.WriteFile('/test/file/to.write', 'the.contents')
  ...
  def testWriteFile_withEchoAndSU(self):
    ...
    with self.assertCalls(
        ...
        (self.call.adb.Shell(expected_cmd),
         '')):
      self.presentation.device.WriteFile('/test/file', 'contents', as_root=True)
...
class DeviceUtilsStatDirectoryTest(DeviceUtilsTest):
  ...
  def getStatEntries(self, path_given='/', path_listed='/'):
    with self.assertCall(
      ...
      entries = self.presentation.device.StatDirectory(path_given)
  ...
  def getListEntries(self):
    with self.assertCall(
        self.call.presentation.device.RunShellCommand(
            ...
            check_return=True, as_root=False, env={'TZ': 'utc'}),
        self.EXAMPLE_LS_OUTPUT):
      return self.presentation.device.ListDirectory('/')
...
class DeviceUtilsStatPathTest(DeviceUtilsTest):
  ...
  def testStatPath_file(self):
    with self.assertCall(
        self.call.presentation.device.StatDirectory('/data/local/tmp', as_root=False),
        self.EXAMPLE_DIRECTORY):
      self.assertEquals(self.INDEX['foo.txt'],
                        self.presentation.device.StatPath('/data/local/tmp/foo.txt'))
  ...
  def testStatPath_directory(self):
    with self.assertCall(
      ...
      self.assertEquals(self.INDEX['some_dir'],
                        self.presentation.device.StatPath('/data/local/tmp/some_dir'))
  ...
  def testStatPath_doesNotExist(self):
    with self.assertCall(
        ...
        self.EXAMPLE_DIRECTORY):
      with self.assertRaises(device_errors.CommandFailedError):
        self.presentation.device.StatPath('/data/local/tmp/does.not.exist.txt')
...
class DeviceUtilsFileSizeTest(DeviceUtilsTest):
  ...
  def testFileSize_file(self):
    with self.assertCall(
      ...
      self.assertEquals(123,
                        self.presentation.device.FileSize('/data/local/tmp/foo.txt'))
  ...
  def testFileSize_doesNotExist(self):
    with self.assertCall(
        ...
        self.EXAMPLE_DIRECTORY):
      with self.assertRaises(device_errors.CommandFailedError):
        self.presentation.device.FileSize('/data/local/tmp/does.not.exist.txt')
  ...
  def testFileSize_directoryWithNoSize(self):
    with self.assertCall(
        ...
        self.EXAMPLE_DIRECTORY):
      with self.assertRaises(device_errors.CommandFailedError):
        self.presentation.device.FileSize('/data/local/tmp/some_dir')
...
class DeviceUtilsSetJavaAssertsTest(DeviceUtilsTest):
  ...
  def testSetJavaAsserts_enable(self):
    with self.assertCalls(
        (self.call.presentation.device.ReadFile(self.presentation.device.LOCAL_PROPERTIES_PATH),
         'some.example.prop=with an example value\n'
         'some.other.prop=value_ok\n'),
        self.call.presentation.device.WriteFile(
            self.presentation.device.LOCAL_PROPERTIES_PATH,
            'some.example.prop=with an example value\n'
            'some.other.prop=value_ok\n'
            'dalvik.vm.enableassertions=all\n'),
        (self.call.presentation.device.GetProp('dalvik.vm.enableassertions'), ''),
        self.call.presentation.device.SetProp('dalvik.vm.enableassertions', 'all')):
      self.assertTrue(self.presentation.device.SetJavaAsserts(True))
  ...
  def testSetJavaAsserts_disable(self):
    with self.assertCalls(
        ...
        self.call.presentation.device.WriteFile(
            ...
            'some.example.prop=with an example value\n'
            'some.other.prop=value_ok\n'),
        (self.call.presentation.device.GetProp('dalvik.vm.enableassertions'), 'all'),
        self.call.presentation.device.SetProp('dalvik.vm.enableassertions', '')):
      self.assertTrue(self.presentation.device.SetJavaAsserts(False))
  ...
  def testSetJavaAsserts_alreadyEnabled(self):
    with self.assertCalls(
        (self.call.presentation.device.ReadFile(self.presentation.device.LOCAL_PROPERTIES_PATH),
         ...
         'some.other.prop=value_ok\n'),
        (self.call.presentation.device.GetProp('dalvik.vm.enableassertions'), 'all')):
      self.assertFalse(self.presentation.device.SetJavaAsserts(True))
...
class DeviceUtilsEnsureCacheInitializedTest(DeviceUtilsTest):
  ...
  def testEnsureCacheInitialized_noCache_success(self):
    self.assertIsNone(self.presentation.device._cache['token'])
    with self.assertCall(
        self.call.presentation.device.RunShellCommand(
            AnyStringWith('getprop'),
            shell=True, check_return=True, large_output=True),
        ['/sdcard', 'TOKEN']):
      self.presentation.device._EnsureCacheInitialized()
    self.assertIsNotNone(self.presentation.device._cache['token'])
  ...
  def testEnsureCacheInitialized_cache(self):
    self.presentation.device._cache['token'] = 'TOKEN'
    with self.assertCalls():
      self.presentation.device._EnsureCacheInitialized()
    self.assertIsNotNone(self.presentation.device._cache['token'])
  ...
  def testGetProp_exists(self):
    with self.assertCall(
        self.call.presentation.device.RunShellCommand(
            ...
            timeout=self.presentation.device._default_timeout,
            retries=self.presentation.device._default_retries),
        'property_value'):
      self.assertEqual('property_value',
                       self.presentation.device.GetProp('test.property'))
  ...
  def testGetProp_doesNotExist(self):
    with self.assertCall(
        self.call.presentation.device.RunShellCommand(
            ...
            retries=self.presentation.device._default_retries),
        ''):
      self.assertEqual('', self.presentation.device.GetProp('property.does.not.exist'))

  def testGetProp_cachedRoProp(self):
    with self.assertCalls(
        self.EnsureCacheInitialized(props=['[ro.build.type]: [userdebug]'])):
      self.assertEqual('userdebug',
                       self.presentation.device.GetProp('ro.build.type', cache=True))
      self.assertEqual('userdebug',
                       self.presentation.device.GetProp('ro.build.type', cache=True))
...
class DeviceUtilsSetPropTest(DeviceUtilsTest):
  ...
  def testSetProp(self):
    with self.assertCall(
      ...
      self.presentation.device.SetProp('test.property', 'test value')

  def testSetProp_check_succeeds(self):
    with self.assertCalls(
        (self.call.presentation.device.RunShellCommand(
            ['setprop', 'test.property', 'new_value'], check_return=True)),
        (self.call.presentation.device.GetProp('test.property', cache=False), 'new_value')):
      self.presentation.device.SetProp('test.property', 'new_value', check=True)

  def testSetProp_check_fails(self):
    with self.assertCalls(
        (self.call.presentation.device.RunShellCommand(
            ['setprop', 'test.property', 'new_value'], check_return=True)),
        (self.call.presentation.device.GetProp('test.property', cache=False), 'old_value')):
      with self.assertRaises(device_errors.CommandFailedError):
        self.presentation.device.SetProp('test.property', 'new_value', check=True)
...
class DeviceUtilsGetPidsTest(DeviceUtilsTest):
  ...
  def testGetPids_sdkGreaterThanNougatMR1(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk,
                         return_value=(version_codes.NOUGAT_MR1 + 1)):
      with self.patch_call(self.call.presentation.device.build_id,
                           return_value='ZZZ99Z'):
        with self.assertCall(
            self.call.presentation.device._RunPipedShellCommand(
                'ps -e | grep -F example.process'), []):
          self.presentation.device.GetPids('example.process')

  def testGetPids_noMatches(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk,
                         return_value=version_codes.LOLLIPOP):
      with self.assertCall(
          self.call.presentation.device._RunPipedShellCommand('ps | grep -F does.not.match'),
          self._grepOutput('does.not.match')):
        self.assertEqual({}, self.presentation.device.GetPids('does.not.match'))

  def testGetPids_oneMatch(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk,
                         return_value=version_codes.LOLLIPOP):
      with self.assertCall(
          self.call.presentation.device._RunPipedShellCommand('ps | grep -F one.match'),
          self._grepOutput('one.match')):
        self.assertEqual(
            {'one.match': ['1001']},
            self.presentation.device.GetPids('one.match'))

  def testGetPids_multipleMatches(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk,
                         return_value=version_codes.LOLLIPOP):
      with self.assertCall(
          self.call.presentation.device._RunPipedShellCommand('ps | grep -F match'),
          self._grepOutput('match')):
        self.assertEqual(
            {'one.match': ['1001'],
             'two.match': ['1002'],
             'three.match': ['1003']},
            self.presentation.device.GetPids('match'))

  def testGetPids_quotable(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk,
                         return_value=version_codes.LOLLIPOP):
      with self.assertCall(
          self.call.presentation.device._RunPipedShellCommand("ps | grep -F 'my$process'"),
          self._grepOutput('my$process')):
        self.assertEqual(
            {'my$process': ['1234']}, self.presentation.device.GetPids('my$process'))

  def testGetPids_multipleInstances(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk,
                         return_value=version_codes.LOLLIPOP):
      with self.assertCall(
          self.call.presentation.device._RunPipedShellCommand('ps | grep -F foo'),
          self._grepOutput('foo')):
        self.assertEqual(
            {'foo': ['1000', '1236']},
            self.presentation.device.GetPids('foo'))
      ...
      with self.assertCall(
        ...
        self.assertEqual(
            {'one.match': ['1001'],
             ...
             'my$process': ['1234'],
             'foo': ['1000', '1236']},
            self.presentation.device.GetPids())
  ...
  def testGetApplicationPids_notFound(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk,
      ...
      with self.assertCall(
        ...
        self.assertEqual([], self.presentation.device.GetApplicationPids('match'))
  ...
  def testGetApplicationPids_foundOne(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk,
      ...
      with self.assertCall(
          self.call.presentation.device._RunPipedShellCommand('ps | grep -F one.match'),
          self._grepOutput('one.match')):
        self.assertEqual(['1001'], self.presentation.device.GetApplicationPids('one.match'))
  ...
  def testGetApplicationPids_foundMany(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk,
      ...
      with self.assertCall(
        ...
        self.assertEqual(
            ['1000', '1236'],
            self.presentation.device.GetApplicationPids('foo'))
  ...
  def testGetApplicationPids_atMostOneNotFound(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk,
      ...
      with self.assertCall(
        ...
        self.assertEqual(
            None,
            self.presentation.device.GetApplicationPids('match', at_most_one=True))
  ...
  def testGetApplicationPids_atMostOneFound(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk,
      ...
      with self.assertCall(
        ...
        self.assertEqual(
            '1001',
            self.presentation.device.GetApplicationPids('one.match', at_most_one=True))
    ...
    with self.patch_call(self.call.presentation.device.build_version_sdk,
      ...
      with self.assertRaises(device_errors.CommandFailedError):
        with self.assertCall(
            self.call.presentation.device._RunPipedShellCommand('ps | grep -F foo'),
            self._grepOutput('foo')):
          self.presentation.device.GetApplicationPids('foo', at_most_one=True)
...
class DeviceUtilsGetSetEnforce(DeviceUtilsTest):
  ...
  def testGetEnforce_Enforcing(self):
    with self.assertCall(self.call.adb.Shell('getenforce'), 'Enforcing'):
      self.assertEqual(True, self.presentation.device.GetEnforce())

  def testGetEnforce_Permissive(self):
    with self.assertCall(self.call.adb.Shell('getenforce'), 'Permissive'):
      self.assertEqual(False, self.presentation.device.GetEnforce())

  def testGetEnforce_Disabled(self):
    with self.assertCall(self.call.adb.Shell('getenforce'), 'Disabled'):
      self.assertEqual(None, self.presentation.device.GetEnforce())

  def testSetEnforce_Enforcing(self):
    with self.assertCalls(
        (self.call.presentation.device.NeedsSU(), False),
        (self.call.adb.Shell('setenforce 1'), '')):
      self.presentation.device.SetEnforce(enabled=True)

  def testSetEnforce_Permissive(self):
    with self.assertCalls(
        (self.call.presentation.device.NeedsSU(), False),
        (self.call.adb.Shell('setenforce 0'), '')):
      self.presentation.device.SetEnforce(enabled=False)

  def testSetEnforce_EnforcingWithInt(self):
    with self.assertCalls(
        (self.call.presentation.device.NeedsSU(), False),
        (self.call.adb.Shell('setenforce 1'), '')):
      self.presentation.device.SetEnforce(enabled=1)

  def testSetEnforce_PermissiveWithInt(self):
    with self.assertCalls(
        (self.call.presentation.device.NeedsSU(), False),
        (self.call.adb.Shell('setenforce 0'), '')):
      self.presentation.device.SetEnforce(enabled=0)
  ...
  def testSetEnforce_PermissiveWithStr(self):
    with self.assertCalls(
        (self.call.presentation.device.NeedsSU(), False),
        (self.call.adb.Shell('setenforce 0'), '')):
      self.presentation.device.SetEnforce(enabled='0')  # Not recommended but it works!
  ...
  def testTakeScreenshot_fileNameProvided(self):
    with self.assertCalls(
        ...
        (self.call.adb.Shell('/system/bin/screencap -p /tmp/path/temp-123.png'),
         ''),
        self.call.presentation.device.PullFile('/tmp/path/temp-123.png',
                                  '/test/host/screenshot.png')):
      self.presentation.device.TakeScreenshot('/test/host/screenshot.png')
...
class DeviceUtilsGetMemoryUsageForPidTest(DeviceUtilsTest):
  ...
  def testGetMemoryUsageForPid_validPid(self):
    with self.assertCalls(
        ...
        (self.call.presentation.device.ReadFile('/proc/1234/status', as_root=True),
         'VmHWM: 1024 kB\n')):
      self.assertEqual(
          {
            'Size': 100,
          ...
          self.presentation.device.GetMemoryUsageForPid(1234))
  ...
  def testGetMemoryUsageForPid_noSmaps(self):
    with self.assertCalls(
        ...
        (self.call.presentation.device.ReadFile('/proc/4321/status', as_root=True),
         'VmHWM: 1024 kb\n')):
      self.assertEquals({'VmHWM': 1024}, self.presentation.device.GetMemoryUsageForPid(4321))
  ...
  def testGetMemoryUsageForPid_noStatus(self):
    with self.assertCalls(
      ...
      self.assertEquals(
          ...
          self.presentation.device.GetMemoryUsageForPid(4321))
...
'''
    with self.assertCalls(
      ...
      package_name = self.presentation.device.DismissCrashDialogIfNeeded()
...
class DeviceUtilsClientCache(DeviceUtilsTest):
  ...
  def testClientCache_twoCaches(self):
    self.presentation.device._cache['test'] = 0
    client_cache_one = self.presentation.device.GetClientCache('ClientOne')
    client_cache_one['test'] = 1
    client_cache_two = self.presentation.device.GetClientCache('ClientTwo')
    client_cache_two['test'] = 2
    self.assertEqual(self.presentation.device._cache['test'], 0)
    self.assertEqual(client_cache_one, {'test': 1})
    self.assertEqual(client_cache_two, {'test': 2})
    self.presentation.device._ClearCache()
    self.assertTrue('test' not in self.presentation.device._cache)
  ...
  def testClientCache_multipleInstances(self):
    client_cache_one = self.presentation.device.GetClientCache('ClientOne')
    client_cache_one['test'] = 1
    client_cache_two = self.presentation.device.GetClientCache('ClientOne')
    self.assertEqual(client_cache_one, {'test': 1})
...
class DeviceUtilsHealthyDevicesTest(mock_calls.TestCase):
  ...
  def testHealthyDevices_emptyBlacklist_defaultDeviceArg(self):
    ...
    for serial, presentation.device in zip(test_serials, devices):
      self.assertTrue(isinstance(presentation.device, device_utils.DeviceUtils))
      self.assertEquals(serial, presentation.device.adb.GetDeviceSerial())
  ...
  def testHealthyDevices_noneDeviceArg_multiple_attached_ANDROID_SERIAL(self):
    try:
      ...
      with self.assertCalls():  # Should skip adb devices when presentation.device is known.
        device_utils.DeviceUtils.HealthyDevices(device_arg=None)
    finally:
...
class DeviceUtilsRestartAdbdTest(DeviceUtilsTest):
  ...
  def testAdbdRestart(self):
    ...
    with self.assertCalls(
        ...
        self.call.presentation.device.WriteFile(mock.ANY, mock.ANY),
        (self.call.presentation.device.RunShellCommand(
            ['source', mock_temp_file], check_return=True, as_root=True)),
        self.call.adb.WaitForDevice()):
      self.presentation.device.RestartAdbd()


class DeviceUtilsGrantPermissionsTest(DeviceUtilsTest):

  def testGrantPermissions_none(self):
    self.presentation.device.GrantPermissions('package', [])

  def testGrantPermissions_underM(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk,
                         return_value=version_codes.LOLLIPOP):
      self.presentation.device.GrantPermissions('package', ['p1'])
  ...
  def testGrantPermissions_multiple(self):
    ...
    with self.patch_call(self.call.presentation.device.build_version_sdk,
      ...
      with self.assertCalls(
          (self.call.presentation.device.RunShellCommand(
              permissions_cmd, shell=True, check_return=True), [])):
        self.presentation.device.GrantPermissions('package', ['p1', 'p2'])
  ...
  def testGrantPermissions_WriteExtrnalStorage(self):
    ...
    with self.patch_call(self.call.presentation.device.build_version_sdk,
      ...
      with self.assertCalls(
          (self.call.presentation.device.RunShellCommand(
              permissions_cmd, shell=True, check_return=True), [])):
        self.presentation.device.GrantPermissions(
  ...
  def testIsScreenOn_onPreL(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk,
      ...
      with self.assertCalls(
        ...
        self.assertTrue(self.presentation.device.IsScreenOn())
  ...
  def testIsScreenOn_offPreL(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk,
      ...
      with self.assertCalls(
        ...
        self.assertFalse(self.presentation.device.IsScreenOn())
  ...
  def testIsScreenOn_noOutput(self):
    with self.patch_call(self.call.presentation.device.build_version_sdk,
      ...
      with self.assertCalls(
        ...
        with self.assertRaises(device_errors.CommandFailedError):
          self.presentation.device.IsScreenOn()
...
class DeviecUtilsSetScreen(DeviceUtilsTest):
  ...
  def testSetScren_alreadySet(self):
    with self.assertCalls(
        (self.call.presentation.device.IsScreenOn(), False)):
      self.presentation.device.SetScreen(False)

  @mock.patch('time.sleep', mock.Mock())
  def testSetScreen_on(self):
    with self.assertCalls(
        (self.call.presentation.device.IsScreenOn(), False),
        (self.call.presentation.device.SendKeyEvent(keyevent.KEYCODE_POWER), None),
        (self.call.presentation.device.IsScreenOn(), True)):
...
class DeviecUtilsLoadCacheData(DeviceUtilsTest):
  ...
  def testTokenMissing(self):
    with self.assertCalls(
      ...
      self.assertFalse(self.presentation.device.LoadCacheData('{}'))

  def testTokenStale(self):
    with self.assertCalls(
        self.EnsureCacheInitialized()):
      self.assertFalse(self.presentation.device.LoadCacheData('{"token":"foo"}'))

  def testTokenMatches(self):
    with self.assertCalls(
        self.EnsureCacheInitialized()):
      self.assertTrue(self.presentation.device.LoadCacheData('{"token":"TOKEN"}'))

  def testDumpThenLoad(self):
    with self.assertCalls(
        self.EnsureCacheInitialized()):
      data = json.loads(self.presentation.device.DumpCacheData())
      data['token'] = 'TOKEN'
      self.assertTrue(self.presentation.device.LoadCacheData(json.dumps(data)))
