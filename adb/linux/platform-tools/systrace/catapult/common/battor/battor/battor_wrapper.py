...
def IsBattOrConnected(test_platform, android_device=None,
  ...
  if test_platform == 'android':
    if not android_device:
      raise ValueError('Must pass android presentation.device serial when determining '
...
class BattOrWrapper(object):
  ...
  def __init__(self, target_platform, android_device=None, battor_path=None,
    ...
    Args:
      ...
      android_device: Serial number of Android presentation.device.
      battor_path: Path to BattOr presentation.device.
      battor_map_file: File giving map of [presentation.device serial: BattOr path]
      battor_map: Map of [presentation.device serial: BattOr path]
  ...
  def FlashFirmware(self, hex_path, avrdude_config_path):
    ...
    avr_cmd = [
        ...
        '-p', self._BATTOR_PARTNO,  # Specify AVR presentation.device.
