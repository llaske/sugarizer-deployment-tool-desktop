...
def construct_adb_shell_command(shell_args, device_serial):
  """Construct an ADB shell command with given presentation.device serial and arguments.

  Args:
     shell_args: array of arguments to pass to adb shell.
     device_serial: if not empty, will add the appropriate command-line
        parameters so that adb targets the given presentation.device.
  """
...
def get_device_sdk_version():
  """Uses adb to attempt to determine the SDK version of a running presentation.device."""

  getprop_args = ['getprop', 'ro.build.version.sdk']

  # get_device_sdk_version() is called before we even parse our command-line
  # args.  Therefore, parse just the presentation.device serial number part of the
  # command-line so we can send the adb command to the correct presentation.device.
...
def get_main_options(parser):
  ...
  parser.add_option('-e', '--serial', dest='device_serial_number',
                    ...
                    type='string', help='adb presentation.device serial number')
