...
class ChromeTracingAgent(tracing_agents.TracingAgent):
  def __init__(self, presentation.device, package_info, ring_buffer, trace_memory=False):
  ...
  @staticmethod
  def GetCategories(presentation.device, package_info):
    with presentation.device.GetLogcatMonitor() as logmon:
      presentation.device.BroadcastIntent(intent.Intent(
...
class ChromeConfig(tracing_agents.TracingConfig):
  def __init__(self, chrome_categories, trace_cc, trace_frame_viewer,
               ...
               trace_scheduler, ring_buffer, presentation.device, package_info):
    ...
    self.presentation.device = presentation.device
    self.package_info = package_info


def try_create_agent(config):
  if config.chrome_categories:
    return ChromeTracingAgent(config.presentation.device, config.package_info,
...
def get_config(options):
  return ChromeConfig(options.chrome_categories, options.trace_cc,
                      ...
                      options.ring_buffer, options.presentation.device,
