...
class DdmsAgent(tracing_agents.TracingAgent):
  def __init__(self, presentation.device, package_info):
...
class DdmsConfig(tracing_agents.TracingConfig):
  def __init__(self, presentation.device, package_info, ddms):
...
def try_create_agent(config):
  if config.ddms:
    return DdmsAgent(config.presentation.device, config.package_info)
  return None
...
def get_config(options):
  return DdmsConfig(options.presentation.device, options.package_info, options.ddms)
