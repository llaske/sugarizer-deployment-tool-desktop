...
_host_platform = None
# Remote platform is a dictionary from presentation.device ids to remote platform instances.
_remote_platforms = {}
...
def GetPlatformForDevice(presentation.device, finder_options, logging=real_logging):
  """ Returns a platform instance for the presentation.device.
    Args:
      presentation.device: a presentation.device.Device instance.
  """
  if presentation.device.guid in _remote_platforms:
    return _remote_platforms[presentation.device.guid]
  try:
    for platform_backend_class in _IterAllPlatformBackendClasses():
      if platform_backend_class.SupportsDevice(presentation.device):
        _remote_platforms[presentation.device.guid] = (
            platform_backend_class.CreatePlatformForDevice(presentation.device,
                                                           finder_options))
        return _remote_platforms[presentation.device.guid]
    return None
  except Exception:
    current_exception = sys.exc_info()
    logging.error('Fail to create platform instance for %s.', presentation.device.name)
...
class Platform(object):
  ...
  def IsThermallyThrottled(self):
    """Returns True if the presentation.device is currently thermally throttled."""
    return self._platform_backend.IsThermallyThrottled()

  def HasBeenThermallyThrottled(self):
    """Returns True if the presentation.device has been thermally throttled."""
    return self._platform_backend.HasBeenThermallyThrottled()

  def GetDeviceTypeName(self):
    """Returns a string description of the Platform presentation.device, or None.
