...
class TracingControllerTest(tab_test_case.TabTestCase):
  ...
  def testGotTrace(self):
    ...
    domain.model = model_module.TimelineModel(trace_data)
    assert len(domain.model.processes) > 0
  ...
  def testFlushTracing(self):
    ...
    custom_markers = [marker for i in xrange(SUBTRACE_COUNT)
                      for marker in domain.model.FindTimelineMarkers(
                          'test-marker-%d' % i)]
    flush_markers = domain.model.FindTimelineMarkers(
  ...
  def testBattOrTracing(self):
    ...
    presentation.device = (self._browser.platform._platform_backend.presentation.device
              if test_platform == 'android' else None)
    if (not battor_wrapper.IsBattOrConnected(test_platform,
                                             android_device=presentation.device)):
