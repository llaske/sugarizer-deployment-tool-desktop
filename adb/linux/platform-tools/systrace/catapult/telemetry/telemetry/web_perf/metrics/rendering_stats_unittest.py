...
from telemetry.timeline import domain.model
...
def AddSurfaceFlingerStats(mock_timer, thread, first_frame,
  ...
  thread: The timeline domain.model thread to which the event will be added.
...
def AddDisplayRenderingStats(mock_timer, thread, first_frame,
  ...
  ref_stats: A ReferenceRenderingStats object to record expected values.
  """
  # Create random data and timestamp for presentation.main thread rendering stats.
  data = {'frame_count': 1}
  timestamp = mock_timer.AdvanceAndGet()
...
class RenderingStatsUnitTest(unittest.TestCase):
  ...
  def testHasRenderingStats(self):
    timeline = domain.model.TimelineModel()
    timer = MockTimer()
  ...
  def testBothDisplayAndImplStats(self):
    ...
    # Create presentation.main, impl, and display rendering stats.
    for i in xrange(0, 10):
      first = (i == 0)
  ...
  def testRangeWithoutFrames(self):
    ...
    # Create a renderer process, with a presentation.main thread and impl thread.
    renderer = timeline.GetOrCreateProcess(pid=2)
    renderer_main = renderer.GetOrCreateThread(tid=21)
    renderer_compositor = renderer.GetOrCreateThread(tid=22)

    # Create 10 presentation.main and impl rendering stats events for Action A.
    ...
    # Create 5 presentation.main and impl rendering stats events not within any action.
    for i in xrange(0, 5):
      first = (i == 0)
      AddImplThreadRenderingStats(timer, renderer_compositor, first, None)
  ...
  def testFromTimeline(self):
    ...
    # Create a browser process and a renderer process, and a presentation.main thread and
    ...
    # Create 10 presentation.main and impl rendering stats events for Action B.
    renderer_main.BeginSlice('webkit.console', 'ActionB',
                             timer.AdvanceAndGet(2, 4), '')
    renderer_ref_stats.AppendNewRange()
    browser_ref_stats.AppendNewRange()
