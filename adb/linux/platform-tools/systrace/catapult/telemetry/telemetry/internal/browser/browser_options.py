...
class BrowserFinderOptions(optparse.Values):
  ...
  def CreateParser(self, *args, **kwargs):
    ...
    group.add_option(
        '--remote',
        dest='cros_remote',
        help='The hostname of a remote ChromeOS presentation.device to use.')
    group.add_option(
        '--remote-ssh-port',
        type=int,
        default=socket.getservbyname('ssh'),
        dest='cros_remote_ssh_port',
        help='The SSH port of the remote ChromeOS presentation.device (requires --remote).')
    identity = None
    ...
    group.add_option('--identity',
        ...
        default=identity,
        help='The identity file to use when ssh\'ing into the ChromeOS presentation.device')
    ...
    group.add_option('--presentation.device',
    help='The presentation.device ID to use. '
    ...
    def ParseArgs(args=None):
      ...
      if self.remote_platform_options.presentation.device == 'list':
        if binary_manager.NeedsInit():
          binary_manager.InitDependencyManager([])
        devices = device_finder.GetDevicesMatchingOptions(self)
        print 'Available devices:'
        for presentation.device in devices:
          print ' ', presentation.device.name
        sys.exit(0)
        ...
        for presentation.device in devices:
          try:
            possible_browsers = browser_finder.GetAllAvailableBrowsers(self,
                                                                       presentation.device)
            browser_types[presentation.device.name] = sorted(
