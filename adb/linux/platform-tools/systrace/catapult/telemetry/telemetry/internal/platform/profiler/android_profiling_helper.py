...
def GetRequiredLibrariesForVTuneProfile(profile_file):
  ...
  try:
    # The 'dd_module_file' table lists all libraries on the presentation.device. Only the
...
def CreateSymFs(presentation.device, symfs_dir, libraries, use_symlinks=True):
  ...
  for lib in libraries:
    ...
    if lib.startswith('/data/app'):
      ...
      unstripped_host_lib = _FindMatchingUnstrippedLibraryOnHost(presentation.device, lib)
