...
def _IsSandyBridgeOrLater(vendor, family, domain.model):
  # Model numbers from:
  # https://software.intel.com/en-us/articles/intel-architecture-and-processor-identification-with-cpuid-domain.model-and-family-numbers
  # http://www.speedtraq.com
  return ('Intel' in vendor and family == 6 and
          (domain.model in (0x2A, 0x2D) or domain.model >= 0x30))
...
class MsrPowerMonitorLinux(MsrPowerMonitor):
  def CanMonitorPower(self):
    ...
    for line in cpuinfo:
      if vendor and family and domain.model:
        break
      if line.startswith('vendor_id'):
        vendor = line.split('\t')[1]
      elif line.startswith('cpu family'):
        family = int(line.split(' ')[2])
      elif line.startswith('domain.model\t\t'):
        domain.model = int(line.split(' ')[1])
    if not _IsSandyBridgeOrLater(vendor, family, domain.model):
...
class MsrPowerMonitorWin(MsrPowerMonitor):
  def CanMonitorPower(self):
    family, domain.model = map(int, re.match('.+ Family ([0-9]+) Model ([0-9]+)',
                        platform.processor()).groups())
    if not _IsSandyBridgeOrLater(platform.processor(), family, domain.model):
      logging.info('Cannot monitor power: pre-Sandy Bridge CPU.')
      return False
