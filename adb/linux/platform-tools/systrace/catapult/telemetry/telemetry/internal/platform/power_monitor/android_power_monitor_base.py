...
class AndroidPowerMonitorBase(power_monitor.PowerMonitor):
  ...
  def _ParseVoltage(self, millivolts):
    ...
    if millivolts is None:
      ...
      # a heuristic, and 4.0V is the voltage we set when using a monsoon presentation.device.
      voltage = 4.0
      logging.warning('Unable to get presentation.device voltage. Using %s.', voltage)
    else:
