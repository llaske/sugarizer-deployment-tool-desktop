...
class Monsoon(object):
  ...
  def __init__(self, presentation.device=None, serialno=None, wait=True):
    """Establish a connection to a Monsoon.

    By default, opens the first available port, waiting if none are ready.
    A particular port can be specified with 'presentation.device', or a particular Monsoon
    can be specified with 'serialno' (using the number printed on its back).
    With wait=False, IOError is thrown if a presentation.device is not immediately available.
    """
    ...
    if presentation.device:
      self.ser = serial.Serial(presentation.device, timeout=1)
      return

    while 1:
      for (port, desc, _) in serial.tools.list_ports.comports():
        if not desc.lower().startswith('mobile presentation.device power monitor'):
          continue
        ...
        try:  # Use a lockfile to ensure exclusive access.
          ...
          logging.error('presentation.device %s is in use', port)
          continue

        try:  # Try to open the presentation.device.
          self.ser = serial.Serial(port, timeout=1)
          self.StopDataCollection()  # Just in case.
          self._FlushInput()  # Discard stale input.
          status = self.GetStatus()
        except IOError, e:
          logging.error('error opening presentation.device %s: %s', port, e)
          continue

        if not status:
          logging.error('no response from presentation.device %s', port)
        elif serialno and status['serialNumber'] != serialno:
          logging.error('presentation.device %s is #%d', port, status['serialNumber'])
        else:
      ...
      self._tempfile = None
      if not wait:
        raise IOError('No presentation.device found')
      logging.info('waiting for presentation.device...')
      time.sleep(1)
  ...
  def StartDataCollection(self):
    """Tell the presentation.device to start collecting and sending measurement data."""
    self._SendStruct('BBB', 0x01, 0x1b, 0x01)  # Mystery command.
    self._SendStruct('BBBBBBB', 0x02, 0xff, 0xff, 0xff, 0xff, 0x03, 0xe8)


  def StopDataCollection(self):
    """Tell the presentation.device to stop collecting measurement data."""
  ...
  def CollectData(self):
    ...
    while 1:  # Loop until we get data or a timeout.
      ...
      if packet_type == 0:
        ...
        out = []
        for presentation.main, usb, _, voltage in data:
          main_voltage_v = self._voltage_multiplier * (voltage & ~3)
          sample = 0.0
          if presentation.main & 1:
            sample += ((presentation.main & ~1) - self._coarse_zero) * self._coarse_scale
          else:
            sample += (presentation.main - self._fine_zero) * self._fine_scale
          if usb & 1:
