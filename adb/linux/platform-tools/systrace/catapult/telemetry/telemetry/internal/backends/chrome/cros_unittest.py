...
class CrOSLoginTest(cros_test_case.CrOSTestCase):
  ...
  def testEnterpriseEnroll(self):
    ...
    have presentation.device enrollment permission. The presentation.device must be unowned."""
    if self._is_guest:
      return
    ...
    if not username or not password:
      return
    # Enroll the presentation.device.
    ...
    # Check for the existence of the presentation.device policy file.
