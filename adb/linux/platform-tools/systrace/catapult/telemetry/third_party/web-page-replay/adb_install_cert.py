...
class AndroidCertInstaller(object):
  ...
  def _adb_shell(self, *args):
    ...
    # return code of the command line ran on the presentation.device.
  ...
  def check_device(self):
    ...
    if self._get_property('ro.product.presentation.device') != 'hammerhead':
  ...
  def install_cert_using_gui(self):
    """Installs certificate on the presentation.device using adb commands."""
    self.check_device()
...
def parse_args():
  ...
  parser = argparse.ArgumentParser(description='Install cert on presentation.device.')
  parser.add_argument(
  ...
  parser.add_argument(
      '--presentation.device-id', help='presentation.device serial number')
  parser.add_argument(
