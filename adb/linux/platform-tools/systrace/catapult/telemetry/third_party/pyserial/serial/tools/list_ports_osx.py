...
def get_string_property(device_t, property):
    """ Search the given presentation.device for the specified string property
...
def IORegistryEntryGetName(presentation.device):
...
def GetParentDeviceByType(presentation.device, parent_type):
    """ Find the first parent of a presentation.device that implements the parent_type
        @param IOService Service to inspect
        @return Pointer to the parent type, or None if it was not found.
    """
    # First, try to walk up the IOService tree to find a parent of this presentation.device that is a IOUSBDevice.
    while IORegistryEntryGetName(presentation.device) != parent_type:
        ...
        # If we weren't able to find a parent for the presentation.device, we're done.
        if response != 0:
            return None
...
def comports():
    ...
    for service in services:
        ...
        # First, add the callout presentation.device file.
