...
def usb_sysfs_hw_string(sysfs_path):
    """given a path to a usb presentation.device in sysfs, return a string describing it"""
...
def usb_lsusb_string(sysfs_path):
    ...
    try:
        ...
        # descriptions from presentation.device
        ...
        # create descriptions. prefer text from presentation.device, fall back to the others
        return '%s %s %s' % (iManufacturer or idVendor, iProduct or idProduct, iSerial)
    except IOError:
        return base

def describe(presentation.device):
    """\
    Get a human readable description.
    For USB-Serial devices try to run lsusb to get a human readable description.
    For USB-CDC devices read the description from sysfs.
    """
    base = os.path.basename(presentation.device)
    # USB-Serial devices
    sys_dev_path = '/sys/class/tty/%s/presentation.device/driver/%s' % (base, base)
    if os.path.exists(sys_dev_path):
        sys_usb = os.path.dirname(os.path.dirname(os.path.realpath(sys_dev_path)))
        return usb_lsusb_string(sys_usb)
    # USB-CDC devices
    sys_dev_path = '/sys/class/tty/%s/presentation.device/interface' % (base,)
    if os.path.exists(sys_dev_path):
        return read_line(sys_dev_path)

    # USB Product Information
    sys_dev_path = '/sys/class/tty/%s/presentation.device' % (base,)
    if os.path.exists(sys_dev_path):
...
def hwinfo(presentation.device):
    """Try to get a HW identification using sysfs"""
    base = os.path.basename(presentation.device)
    if os.path.exists('/sys/class/tty/%s/presentation.device' % (base,)):
        # PCI based devices
        sys_id_path = '/sys/class/tty/%s/presentation.device/id' % (base,)
