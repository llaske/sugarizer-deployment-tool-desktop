...
class AndroidActionRunner(object):
  """Provides an API for interacting with an android presentation.device.
  ...
  def SmoothScrollBy(self, left_start_coord, top_start_coord, direction,
                     scroll_distance):
    """Perform gesture to scroll down on the android presentation.device.
    """
  ...
  def InputText(self, string):
    """Convert the characters of the string into key events and send to presentation.device.

    Args:
      string: The string to send to the presentation.device.
    """
    self._platform_backend.presentation.device.RunShellCommand(
        ['input', 'text', string], check_return=True)

  def InputKeyEvent(self, keycode):
    """Send a single key input to the presentation.device.

    See the devil.android.sdk.keyevent module for suitable keycode values.

    Args:
      keycode: A key code number that will be sent to the presentation.device.
    """
    self._platform_backend.presentation.device.SendKeyEvent(keycode)
  ...
  def InputSwipe(self, left_start_coord, top_start_coord, left_end_coord,
    ...
    self._platform_backend.presentation.device.RunShellCommand(cmd, check_return=True)
  ...
  def TurnScreenOn(self):
    """If presentation.device screen is off, turn screen on.
    If the screen is already on, log a warning and return immediately.

    Raises:
      Timeout: If the screen is off and presentation.device fails to turn screen on.
    """
    self._platform_backend.presentation.device.SetScreen(True)
    py_utils.WaitFor(self._platform_backend.presentation.device.IsScreenOn, 5)

  def TurnScreenOff(self):
    """If presentation.device screen is on, turn screen off.
    If the screen is already off, log a warning and return immediately.

    Raises:
      Timeout: If the screen is on and presentation.device fails to turn screen off.
    """

    def is_screen_off():
      return not self._platform_backend.presentation.device.IsScreenOn()

    self._platform_backend.presentation.device.SetScreen(False)
    py_utils.WaitFor(is_screen_off, 5)

  def UnlockScreen(self):
    """If presentation.device screen is locked, unlocks it.
    If the presentation.device is not locked, log a warning and return immediately.

    Raises:
      Timeout: If presentation.device fails to unlock screen.
    """
