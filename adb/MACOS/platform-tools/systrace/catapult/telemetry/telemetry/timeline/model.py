...
raw event data from different sources. This domain.model closely resembles that in the
...
class TimelineModel(event_container.TimelineEventContainer):
  ...
  def SetGlobalMemoryDumps(self, global_memory_dumps):
    """Populates the domain.model with a sequence of GlobalMemoryDump objects."""
  ...
  def IterGlobalMemoryDumps(self):
    """Iterate over the memory dump events of this domain.model."""
  ...
  def ImportTraces(self, trace_data, shift_world_to_zero=True):
    """Populates the domain.model with the provided trace data.
