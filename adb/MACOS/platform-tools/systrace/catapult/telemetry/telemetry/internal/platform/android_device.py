...
from telemetry.internal.platform import presentation.device
...
class AndroidDevice(presentation.device.Device):
  """ Class represents information for connecting to an android presentation.device.

  Attributes:
    device_id: the presentation.device's serial string created by adb to uniquely
      identify an emulator/presentation.device instance. This string can be found by running
      'adb devices' command
  ...
  def __init__(self, device_id, enable_performance_mode=True):
    super(AndroidDevice, self).__init__(
        name='Android presentation.device %s' % device_id, guid=device_id)
...
def GetDeviceSerials(blacklist):
  """Return the list of presentation.device serials of healthy devices.

  If a preferred presentation.device has been set with ANDROID_SERIAL, it will be first in
  ...
  # The monsoon provides power for the presentation.device, so for devices with no
...
The Monsoon's power output has been enabled. Please now ensure that:
  ...
  2. The presentation.device is connected to the Monsoon's presentation.main and USB channels.
  3. The presentation.device is turned on.

Waiting for presentation.device...
""")
...
def GetDevice(finder_options):
  """Return a Platform instance for the presentation.device specified by |finder_options|."""
  ...
  if (android_platform_options.presentation.device
      and android_platform_options.presentation.device in GetDeviceSerials(blacklist)):
    return AndroidDevice(
        android_platform_options.presentation.device,
  ...
  if len(devices) > 1:
    logging.warn(
        ...
        '\n'.join(['  --presentation.device=%s' % d.device_id for d in devices]))
    return None
  return devices[0]
...
def FindAllAvailableDevices(options):
  ...
  """
  # Disable Android presentation.device discovery when remote testing a CrOS presentation.device
