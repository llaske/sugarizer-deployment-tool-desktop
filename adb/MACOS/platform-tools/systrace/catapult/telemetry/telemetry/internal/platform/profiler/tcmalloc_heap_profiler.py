...
class _TCMallocHeapProfilerAndroid(object):
  """An internal class to set android properties and fetch dumps from presentation.device."""
  ...
  def _SetDeviceProperties(self, properties):
    ...
    self._browser_backend.presentation.device.EnableRoot()
    for values in properties.itervalues():
      device_property = self._browser_backend.presentation.device.GetProp(values[0])
      if not device_property or not device_property.strip():
        self._browser_backend.presentation.device.SetProp(values[0], values[1])
        device_configured = True
    if not self._browser_backend.presentation.device.FileExists(
        self._DEFAULT_DEVICE_DIR):
      self._browser_backend.presentation.device.RunShellCommand(
  ...
  def CollectProfile(self):
    try:
      self._browser_backend.presentation.device.PullFile(
