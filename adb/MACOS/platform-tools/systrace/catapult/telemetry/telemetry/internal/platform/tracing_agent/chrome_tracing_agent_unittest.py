...
class ChromeTracingAgentTest(unittest.TestCase):
  ...
  def testCreateAndRemoveTraceConfigFileOnAndroid(self):
    ...
    self.assertTrue(platform_backend.presentation.device.PathExists(agent.trace_config_file))
    config_file_str = platform_backend.presentation.device.ReadFile(agent.trace_config_file,
                                                       as_root=True)
    ...
    self.assertFalse(platform_backend.presentation.device.PathExists(config_file_path))
    self.assertIsNone(agent.trace_config_file)
