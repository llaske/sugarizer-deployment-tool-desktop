...
def _MonitorPower(presentation.device, is_collecting, output):
  """Monitoring process
     Args:
       presentation.device: A profiler.monsoon object to collect samples from.
  ...
  with output:
    ...
    end_time = None
    try:
      presentation.device.StartDataCollection()
      is_collecting.set()
      # First sample also calibrate the computation.
      presentation.device.CollectData()
      start_time = time.time()
      while is_collecting.is_set():
        new_data = presentation.device.CollectData()
        assert new_data, 'Unable to collect data from presentation.device'
        samples += new_data
      end_time = time.time()
    finally:
      presentation.device.StopDataCollection()
    result = {
