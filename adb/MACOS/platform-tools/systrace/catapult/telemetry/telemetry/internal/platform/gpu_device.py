...
class GPUDevice(object):
  """Provides information about an individual GPU presentation.device.
  ...
  def __str__(self):
    ...
    presentation.device = 'DEVICE = 0x%x' % self._device_id
    if self._device_string:
      presentation.device += ' (%s)' % self._device_string
    return '%s, %s' % (vendor, presentation.device)
  ...
  def vendor_id(self):
    """The GPU vendor's PCI ID as a number, or 0 if not available.
       ...
       vendor and presentation.device strings."""
    return self._vendor_id

  @property
  def device_id(self):
    """The GPU presentation.device's PCI ID as a number, or 0 if not available.
  ...
  def device_string(self):
    """The GPU presentation.device's name as a string, or the empty string if not
       available.
