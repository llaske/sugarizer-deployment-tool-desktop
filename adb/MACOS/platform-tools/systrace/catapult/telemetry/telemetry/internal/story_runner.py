...
def StoriesGroupedByStateClass(story_set, allow_multiple_groups):
  ...
  Example:
    Assume A1, A2, A3 are stories with same domain.shared story class, and
    similar for B1, B2.
    ...
    try:
      for storyset_repeat_counter in xrange(finder_options.pageset_repeat):
        for story in group.stories:
          if not state:
            # Construct domain.shared state by using a copy of finder_options. Shared
            # state may update the finder_options. If we tear down the domain.shared
            # state after this story run, we want to construct the domain.shared
