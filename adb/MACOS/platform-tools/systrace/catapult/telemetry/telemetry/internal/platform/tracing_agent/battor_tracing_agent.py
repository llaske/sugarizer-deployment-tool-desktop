...
class BattOrTracingAgent(tracing_agent.TracingAgent):
  """A tracing agent for getting power data from a BattOr presentation.device.
  ...
  def __init__(self, platform_backend):
    ...
    android_device = (
        platform_backend.presentation.device if platform_backend.GetOSName() == 'android'
        else None)
    self._battery = (
        battery_utils.BatteryUtils(platform_backend.presentation.device)
  ...
  def IsSupported(cls, platform_backend):
    ...
    if platform_backend.GetOSName() == 'android':
      # TODO(rnephew): When we pass BattOr presentation.device map into Telemetry, change
      # this to reflect that.
      return battor_wrapper.IsBattOrConnected(
          'android', android_device=platform_backend.presentation.device)
