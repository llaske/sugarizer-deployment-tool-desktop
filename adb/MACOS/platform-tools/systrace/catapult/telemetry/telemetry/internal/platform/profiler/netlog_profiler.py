...
class NetLogProfiler(profiler.Profiler):
  ...
  def CollectProfile(self):
    ...
    if self._platform_backend.GetOSName() == 'android':
      ...
      self._browser_backend.presentation.device.PullFile(output_file, host_output_file)
      # Clean the presentation.device
      self._browser_backend.presentation.device.RemovePath(output_file)
