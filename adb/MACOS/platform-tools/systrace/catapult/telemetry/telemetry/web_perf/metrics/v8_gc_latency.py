...
class V8GCLatency(timeline_based_metric.TimelineBasedMetric):
  ...
  def AddResults(self, domain.model, renderer_thread, interaction_records, results):
    ...
    self._AddV8MetricsToResults(domain.model, interaction_records, results)

  def _AddV8MetricsToResults(self, domain.model,
                             interaction_records, results):
    self._AddV8EventStatsToResults(domain.model, interaction_records, results)

  def _AddV8EventStatsToResults(self, domain.model, interactions, results):
    v8_event_stats = [
