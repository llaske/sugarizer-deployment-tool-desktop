...
class JitterTimelineMetric(timeline_based_metric.TimelineBasedMetric):
  ...
  This jitter is due to the presentation.main thread attempting to fix the position of a
  ...
  def AddResults(self, domain.model, renderer_thread, interactions, results):
    assert interactions

    jitter_events = []
    for event in domain.model.IterAllEvents(
