...
class TraceEventStatsUnittest(unittest.TestCase):
  ...
  def setUp(self):
    self.domain.model = model_module.TimelineModel()
    self.renderer_process = self.domain.model.GetOrCreateProcess(1)
    self.renderer_process.name = RENDERER_PROCESS
    self.main_thread = self.renderer_process.GetOrCreateThread(tid=11)
    self.other_process = self.domain.model.GetOrCreateProcess(2)
  ...
  def RunAggregator(self, aggregator, interactions):
    ...
    aggregator.AddResults(self.domain.model, self.renderer_process,
