...
class _DummyAdb(object):
  ...
  def Push(self, host_path, device_path):
    logging.debug('(presentation.device %s) pushing %r to %r', self, host_path, device_path)

  def IsOnline(self):
    logging.debug('(presentation.device %s) checking presentation.device online', self)
    return True

  def Shell(self, cmd):
    logging.debug('(presentation.device %s) running command %r', self, cmd)
    return "nice output\n"

  def Reboot(self):
    logging.debug('(presentation.device %s) rebooted!', self)

  @property
  def build_version_sdk(self):
    logging.debug('(presentation.device %s) getting build_version_sdk', self)
...
class TestCaseWithAssertCallsTest(mock_calls.TestCase):
  ...
  def ShellError(self):
    def action(cmd):
      raise ValueError('(presentation.device %s) command %r is not nice' % (self.adb, cmd))
    return action
