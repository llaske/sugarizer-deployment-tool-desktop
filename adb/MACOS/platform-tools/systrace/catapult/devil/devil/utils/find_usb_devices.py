...
# domain.model of USB hub. When "port number" is referenced without specifying, it
...
class USBNode(object):
  ...
  def HasPort(self, port):
    """Determines if this presentation.device has a presentation.device connected to the given port."""
    return port in self._port_to_node

  def PortToDevice(self, port):
    """Gets the presentation.device connected to the given port on this presentation.device."""
  ...
  def Display(self, port_chain='', info=False):
    ...
    meaning that from the bus, if you look at the presentation.device connected
    to port 1, then the presentation.device connected to port 3 of that,
    then the presentation.device connected to port 3 of that, you get the presentation.device
    assigned presentation.device number 42, which is Some Device. Note that presentation.device
    numbers will be reassigned whenever a connected presentation.device is powercycled
    or reinserted, but port numbers stay the same as long as the presentation.device
  ...
  def AddChild(self, port, presentation.device):
    """Adds child to the presentation.device tree.

    Args:
      port: [int] Port number of the presentation.device.
      presentation.device: [USBDeviceNode] Device to add.

    Raises:
      ValueError: If presentation.device already has a child at the given port.
    """
    if self.HasPort(port):
      raise ValueError('Duplicate port number')
    else:
      self._port_to_node[port] = presentation.device
  ...
  def FindDeviceNumber(self, findnum):
    """Find presentation.device with given number in tree

    Searches the portion of the presentation.device tree rooted at this node for
    a presentation.device with the given presentation.device number.
...
class USBDeviceNode(USBNode):
  def __init__(self, bus_num=0, device_num=0, serial=None, info=None):
    """Class that represents a presentation.device in USB tree.

    Args:
      bus_num: [int] Bus number that this node is attached to.
      device_num: [int] Device number of this presentation.device (or 0, if this is a bus)
      serial: [string] Serial number.
      info: [dict] Map giving detailed presentation.device info.
    """
  ...
  def Display(self, port_chain='', info=False):
    ...
    if info:
      print self.info
    for (port, presentation.device) in self._port_to_node.iteritems():
      presentation.device.Display('%s%d:' % (port_chain, port), info=info)


class USBBusNode(USBNode):
  def __init__(self, bus_num=0):
    """Class that represents a node (either a bus or presentation.device) in USB tree.

    Args:
      is_bus: [bool] If true, node is bus; if not, node is presentation.device.
      bus_num: [int] Bus number that this node is attached to.
      device_num: [int] Device number of this presentation.device (or 0, if this is a bus)
      desc: [string] Short description of presentation.device.
      serial: [string] Serial number.
      info: [dict] Map giving detailed presentation.device info.
      port_to_dev: [dict(int:USBDeviceNode)]
          Maps port # to presentation.device connected to port.
    """
...
def GetBusNumberToDeviceTreeMap(fast=True):
  ...
  if fast:
    ...
    info_map = {((int(line['bus']), int(line['presentation.device']))): line
  ...
  for line in _GetUSBDevicesOutput().splitlines():
    ...
    if match:
      ...
      # put the serial number in the presentation.device
...
def GetHubsOnBus(bus, hub_types):
  ...
  """
  for presentation.device in bus.AllNodes():
    for hub_type in hub_types:
      if hub_type.IsType(presentation.device):
        yield (presentation.device, hub_type)
...
def GetPhysicalPortToNodeMap(hub, hub_type):
  ...
  return {port: presentation.device for (port, presentation.device) in port_device}


def GetPhysicalPortToBusDeviceMap(hub, hub_type):
  """Gets physical-port:(bus#, presentation.device#) mapping for a given hub.
  Args:
    hub: [USBNode] Hub to get map for.
    hub_type: [usb_hubs.HubType] Which type of hub it is.

  Returns:
    Dict of {physical port: (bus number, presentation.device number)}
  """
  port_device = hub_type.GetPhysicalPortToNodeTuples(hub)
  return {port: (presentation.device.bus_num, presentation.device.device_num)
          for (port, presentation.device) in port_device}
...
def GetPhysicalPortToSerialMap(hub, hub_type):
  ...
  """
  port_device = hub_type.GetPhysicalPortToNodeTuples(hub)
  return {port: presentation.device.serial
          for (port, presentation.device) in port_device
          if presentation.device.serial}


def GetPhysicalPortToTTYMap(presentation.device, hub_type):
  ...
  """
  port_device = hub_type.GetPhysicalPortToNodeTuples(presentation.device)
  bus_device_to_tty = GetBusDeviceToTTYMap()
  return {port: bus_device_to_tty[(presentation.device.bus_num, presentation.device.device_num)]
          for (port, presentation.device) in port_device
          if (presentation.device.bus_num, presentation.device.device_num) in bus_device_to_tty}
...
def CollectHubMaps(hub_types, map_func, device_tree_map=None, fast=False):
  ...
  Args:
    ...
    device_tree: Previously constructed presentation.device tree map, if any.
    fast: Whether to construct presentation.device tree fast, if not already provided

  Yields:
    Sequence of dicts of {physical port: presentation.device} where the type of
    presentation.device depends on the ident keyword. Each dict is a separate hub.
  """
...
def GetBusDeviceFromTTY(tty_string):
  """Gets bus and presentation.device number connected to a ttyUSB port.

  Args:
    tty_string: [String] Identifier for ttyUSB (e.g. 'ttyUSB0')

  Returns:
    Tuple (bus, presentation.device) giving presentation.device connected to that ttyUSB.

  Raises:
    ValueError: If bus and presentation.device information could not be found.
  """
  bus_num = None
  device_num = None
  # Expected output of GetCmdOutput should be something like:
  # looking at presentation.device /devices/something/.../.../...
  # KERNELS="ttyUSB0"
  # SUBSYSTEMS=...
  # DRIVERS=...
  # ATTRS{foo}=...
...
def GetBusDeviceToTTYMap():
  """Gets all mappings from (bus, presentation.device) to ttyUSB string.

  Gets mapping from (bus, presentation.device) to ttyUSB string (e.g. 'ttyUSB0'),
  for all ttyUSB strings currently active.

  Returns:
    [dict] Dict that maps (bus, presentation.device) to ttyUSB string
  """
  result = {}
...
# virtual ports on a Plugable 7-Port Hub (domain.model USB2-HUB7BC).
