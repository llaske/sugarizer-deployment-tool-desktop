...
class BatteryUtilsTest(mock_calls.TestCase):
  ...
  def setUp(self):
    ...
    self.battery = battery_utils.BatteryUtils(
        self.presentation.device, default_timeout=10, default_retries=0)
  ...
  def testGetNetworkData_noDataUsage(self):
    with self.assertCalls(
        (self.call.presentation.device.RunShellCommand(
            ...
            check_return=True, large_output=True),
         _DUMPSYS_OUTPUT),
        (self.call.presentation.device.ReadFile('/proc/uid_stat/1000/tcp_snd'),
            self.ShellError()),
        (self.call.presentation.device.ReadFile('/proc/uid_stat/1000/tcp_rcv'),
            self.ShellError())):
  ...
  def testGetNetworkData_packageNotCached(self):
    with self.assertCalls(
        (self.call.presentation.device.RunShellCommand(
            ...
            check_return=True, large_output=True),
         _DUMPSYS_OUTPUT),
        (self.call.presentation.device.ReadFile('/proc/uid_stat/1000/tcp_snd'), 1),
        (self.call.presentation.device.ReadFile('/proc/uid_stat/1000/tcp_rcv'), 2)):
...
class BatteryUtilsGetFuelGaugeChargeCounterTest(BatteryUtilsTest):
  ...
  def testGetFuelGaugeChargeCounter_fuelGaugePresent(self):
    ...
    with self.assertCalls(
        ...
        (self.call.presentation.device.ReadFile(mock.ANY), '123')):
...
class BatteryUtilsDiscoverDeviceProfile(BatteryUtilsTest):
  ...
  def testDiscoverDeviceProfile_known(self):
    with self.patch_call(self.call.presentation.device.product_model,
