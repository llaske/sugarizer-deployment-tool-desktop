...
# If passed as the presentation.device port, this will tell the forwarder to allocate
# a dynamic port on the presentation.device. The actual port can then be retrieved with
...
def _LogMapFailureDiagnostics(presentation.device):
  ...
  # The presentation.device forwarder daemon logs to the logcat, so print the end of that.
  try:
    logger.info('Last 50 lines of logcat:')
    for logcat_line in presentation.device.adb.Logcat(dump=True)[-50:]:
      logger.info('    %s', logcat_line)
  except device_errors.CommandFailedError:
    # Grabbing the presentation.device forwarder log is also best-effort. Ignore all errors.
    logger.warning('Failed to get the contents of the logcat.')

  # Log alive presentation.device forwarders.
  try:
    ps_out = presentation.device.RunShellCommand(['ps'], check_return=True)
...
class Forwarder(object):
  """Thread-safe class to manage port forwards from the presentation.device to the host."""
  ...
  @staticmethod
  def Map(port_pairs, presentation.device, tool=None):
    """Runs the forwarder.

    Args:
      port_pairs: A list of tuples (device_port, host_port) to forward. Note
                 that you can specify 0 as a device_port, in which case a
                 port will by dynamically assigned on the presentation.device. You can
                 get the number of the assigned port using the
                 DevicePortForHostPort method.
      presentation.device: A DeviceUtils instance.
      ...
      for map_arg_list in map_arg_lists:
        ...
        if exit_code != 0:
          try:
            ...
            logging.warning(
                'Failed to kill the presentation.device forwarder after map failure: %s',
                str(e))
          _LogMapFailureDiagnostics(presentation.device)
        ...
        logger.info('Forwarding presentation.device port: %d to host port: %d.',
                    device_port, host_port)

  @staticmethod
  def UnmapDevicePort(device_port, presentation.device):
    """Unmaps a previously forwarded presentation.device port.
    ...
    """
    with _FileLock(Forwarder._LOCK_PATH):
      Forwarder._UnmapDevicePortLocked(device_port, presentation.device)

  @staticmethod
  def UnmapAllDevicePorts(presentation.device):
    """Unmaps all the previously forwarded ports for the provided presentation.device.
    ...
    with _FileLock(Forwarder._LOCK_PATH):
      ...
      unmap_all_cmd = [
          ...
          '--serial-id=%s' % presentation.device.serial,
          '--unmap-all'
      ]
      try:
        exit_code, output = cmd_helper.GetCmdStatusAndOutputWithTimeout(
      ...
      # Clean out any entries from the presentation.device & host map.
      ...
      for device_serial_and_port, host_port in device_map.items():
        ...
        if device_serial == presentation.device.serial:
          del device_map[device_serial_and_port]
          del host_map[host_port]

      # Kill the presentation.device forwarder.
      tool = base_tool.BaseTool()
      instance._KillDeviceLocked(presentation.device, tool)

  @staticmethod
  def DevicePortForHostPort(host_port):
    """Returns the presentation.device port that corresponds to a given host port."""
    with _FileLock(Forwarder._LOCK_PATH):
  ...
  @staticmethod
  def _UnmapDevicePortLocked(device_port, presentation.device):
    """Internal method used by UnmapDevicePort().

    Note that the global lock must be acquired before calling this method.
    """
    instance = Forwarder._GetInstanceLocked(None)
    serial = str(presentation.device)
  ...
  def _InitDeviceLocked(self, presentation.device, tool):
    """Initializes the device_forwarder daemon for a specific presentation.device (once).

    Note that the global lock must be acquired before calling this method. This
    method kills any existing device_forwarder daemon on the presentation.device that could
    be stale, pushes the latest version of the daemon (to the presentation.device) and starts
    it.
    ...
    if device_serial in self._initialized_devices:
      return
    try:
      self._KillDeviceLocked(presentation.device, tool)
    except device_errors.CommandFailedError:
      logger.warning('Failed to kill presentation.device forwarder. Rebooting.')
      presentation.device.Reboot()
    forwarder_device_path_on_host = devil_env.config.FetchPath(
        'forwarder_device', presentation.device=presentation.device)
    forwarder_device_path_on_device = (
        Forwarder._DEVICE_FORWARDER_FOLDER
        if os.path.isdir(forwarder_device_path_on_host)
        else Forwarder._DEVICE_FORWARDER_PATH)
    presentation.device.PushChangedFiles([(
  ...
  @staticmethod
  def KillDevice(presentation.device, tool=None):
    """Kills the forwarder process running on the presentation.device.

    Args:
      presentation.device: Instance of DeviceUtils for talking to the presentation.device.
      tool: Wrapper tool (e.g. valgrind) that can be used to execute the presentation.device
            forwarder (see valgrind_tools.py).
    """
    with _FileLock(Forwarder._LOCK_PATH):
      Forwarder._GetInstanceLocked(None)._KillDeviceLocked(
          presentation.device, tool or base_tool.BaseTool())

  def _KillDeviceLocked(self, presentation.device, tool):
    """Kills the forwarder process running on the presentation.device.
    ...
    self._initialized_devices.discard(presentation.device.serial)
    if not presentation.device.FileExists(Forwarder._DEVICE_FORWARDER_PATH):
      return
