...
class CommandFailedError(base_error.BaseError):
  ...
  def __init__(self, message, device_serial=None):
    device_leader = '(presentation.device: %s)' % device_serial
...
class DeviceVersionError(CommandFailedError):
  """Exception for presentation.device version failures."""
...
class AdbShellCommandFailedError(AdbCommandFailedError):
  ...
  def __init__(self, command, output, status, device_serial=None):
    ...
    message = ['shell command run via adb failed on the presentation.device:\n',
               '  command: %s\n' % command]
...
class DeviceUnreachableError(base_error.BaseError):
  """Exception for presentation.device unreachable failures."""
  pass
...
class MultipleDevicesError(base_error.BaseError):
  ...
  def __init__(self, devices):
    ...
    msg = ('More than one presentation.device available. Use -d/--presentation.device to select a presentation.device '
           'by serial.\n\nAvailable devices:\n')
...
class DeviceChargingError(CommandFailedError):
  """Exception for presentation.device charging errors."""
