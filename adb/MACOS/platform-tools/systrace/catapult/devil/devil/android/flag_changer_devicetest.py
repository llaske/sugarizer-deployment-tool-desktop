...
class FlagChangerTest(device_test_case.DeviceTestCase):
  ...
  def tearDown(self):
    ...
    self.presentation.device.RemovePath(
        [self.cmdline_path, self.cmdline_path_legacy], force=True, as_root=True)

  def testFlagChanger_restoreFlags(self):
    if not self.presentation.device.HasRoot():
      self.skipTest('Test needs a rooted presentation.device')

    # Write some custom chrome command line flags.
    self.presentation.device.WriteFile(
    ...
    self.assertTrue(self.presentation.device.PathExists(self.cmdline_path_legacy))

    changer = flag_changer.FlagChanger(self.presentation.device, _CMDLINE_FILE)

    # Legacy command line file is removed, ensuring Chrome picks up the
    # right file.
    self.assertFalse(self.presentation.device.PathExists(self.cmdline_path_legacy))
  ...
  def testFlagChanger_removeFlags(self):
    self.presentation.device.RemovePath(self.cmdline_path, force=True)
    self.assertFalse(self.presentation.device.PathExists(self.cmdline_path))

    with flag_changer.CustomCommandLineFlags(
        self.presentation.device, _CMDLINE_FILE, ['--some', '--flags']):
      self.assertTrue(self.presentation.device.PathExists(self.cmdline_path))
