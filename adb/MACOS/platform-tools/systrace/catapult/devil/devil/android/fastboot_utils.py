...
"""Provides a variety of presentation.device interactions based on fastboot."""
  ...
  for partition in partitions:
    ...
    if image_file:
      ...
      raise device_errors.FastbootCommandFailedError(
          'Failed to flash presentation.device. Could not find image for %s.',
...
class FastbootUtils(object):
  ...
  def __init__(self, presentation.device, fastbooter=None, default_timeout=_DEFAULT_TIMEOUT,
               default_retries=_DEFAULT_RETRIES):
    """FastbootUtils constructor.

    Example Usage to flash a presentation.device:
  ...
  def Reboot(
    ...
    then blocks until the presentation.device is ready.
  ...
  def _VerifyBoard(self, directory):
    """Validate as best as possible that the android build matches the presentation.device.
    ...
    if self._BOARD_VERIFICATION_FILE in files:
      ...
      logger.warning('%s not found. Unable to use it to verify presentation.device.',
                     self._BOARD_VERIFICATION_FILE)
    ...
    if not self._VerifyBoard(directory):
      if force:
        logger.warning('Could not verify build is meant to be installed on '
                       'the current presentation.device type, but force flag is set. '
                       'Flashing presentation.device. Possibly dangerous operation.')
      else:
        raise device_errors.CommandFailedError(
            'Could not verify build is meant to be installed on the current '
            'presentation.device type. Run again with force=True to force flashing with an '
            'unverified board.')
  ...
  def FlashDevice(self, directory, partitions=None, wipe=False):
    """Flash presentation.device with build in |directory|.
    ...
    # If a presentation.device is wiped, then it will no longer have adb keys so it cannot be
    # communicated with to verify that it is rebooted. It is up to the user of
    # this script to ensure that the adb keys are set on the presentation.device after using
    # this to wipe a presentation.device.
