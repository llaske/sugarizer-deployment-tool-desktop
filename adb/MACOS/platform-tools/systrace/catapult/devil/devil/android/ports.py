...
"""Functions that deal with local and presentation.device ports."""
...
def IsDevicePortUsed(presentation.device, device_port, state=''):
  """Checks whether the specified presentation.device port is used or not.

  Args:
    presentation.device: A DeviceUtils instance.
    device_port: Port on presentation.device we want to check.
    state: String of the specified state. Default is empty string, which
           means any state.

  Returns:
    True if the port on presentation.device is already used, otherwise returns False.
  """
  base_urls = ('127.0.0.1:%d' % device_port, 'localhost:%d' % device_port)
  netstat_results = presentation.device.RunShellCommand(
