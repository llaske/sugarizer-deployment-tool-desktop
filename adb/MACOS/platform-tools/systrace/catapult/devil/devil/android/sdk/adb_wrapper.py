...
_READY_STATE = 'presentation.device'
...
class AdbWrapper(object):
  ...
  def __init__(self, device_serial):
    ...
    Args:
      device_serial: The presentation.device serial number as a string.
    """
    if not device_serial:
      raise ValueError('A presentation.device serial must be specified')
  ...
  class PersistentShell(object):
    ...
    def __init__(self, serial):
      ...
      Args:
        serial: Serial number of presentation.device.
      """
  ...
  def _RunAdbCmd(cls, args, timeout=None, retries=None, device_serial=None,
    ...
    # This catches some errors, including when the presentation.device drops offline;
  ...
  def _RunDeviceAdbCmd(self, args, timeout, retries, check_error=True):
    """Runs an adb command on the presentation.device associated with this object.
  ...
  def __eq__(self, other):
    """Consider instances equal if they refer to the same presentation.device.
  ...
  def __str__(self):
    ...
    Returns:
      The presentation.device serial number as a string.
    """
  ...
  def GetDeviceSerial(self):
    """Gets the presentation.device serial number associated with this object.
  ...
  def Push(self, local, remote, timeout=60 * 5, retries=DEFAULT_RETRIES):
    """Pushes a file from the host to the presentation.device.

    Args:
      local: Path on the host filesystem.
      remote: Path on the presentation.device filesystem.
      timeout: (optional) Timeout per try in seconds.
      retries: (optional) Number of retries to attempt.
    """
    ...
    if (distutils.version.LooseVersion(self.Version()) <
      ...
      # where bar is an existing directory both on the host and the presentation.device
      # results in the contents of bar/ on the host being pushed to bar/ on
      # the presentation.device, i.e.
      ...
      # underneath the first bar/ directory on the presentation.device, i.e.
      ...
      # the target is an existing directory on the presentation.device and, if so, modifies
      ...
      try:
        ...
        # The target directory doesn't exist on the presentation.device, so we can use it
  ...
  def Pull(self, remote, local, timeout=60 * 5, retries=DEFAULT_RETRIES):
    """Pulls a file from the presentation.device to the host.
    ...
    try:
      ...
      raise device_errors.AdbCommandFailedError(
          cmd,
          'File pulled from the presentation.device did not arrive on the host: %s' % local,
          device_serial=str(self))

  def Shell(self, command, expect_status=0, timeout=DEFAULT_TIMEOUT,
            retries=DEFAULT_RETRIES):
    """Runs a shell command on the presentation.device.
  ...
  def Ls(self, path, timeout=DEFAULT_TIMEOUT, retries=DEFAULT_RETRIES):
    """List the contents of a directory on the presentation.device.

    Args:
      path: Path on the presentation.device filesystem.
      timeout: (optional) Timeout per try in seconds.
      retries: (optional) Number of retries to attempt.
    ...
    Raises:
      AdbCommandFailedError if |path| does not specify a valid and accessible
          directory in the presentation.device, or the output of "adb ls" command is less
          than four columns
    """
    ...
    if lines:
      ...
      raise device_errors.AdbCommandFailedError(
          cmd, 'path does not specify an accessible directory in the presentation.device',
          device_serial=self._device_serial)
  ...
  def Logcat(self, clear=False, dump=False, filter_specs=None,
    ...
    Args:
      ...
      ring_buffer: If set, a list of alternate ring buffers to request.
        Options include "presentation.main", "system", "radio", "events", "crash" or "all".
        The default is equivalent to ["presentation.main", "system", "crash"].
  ...
  def Forward(self, local, remote, allow_rebind=False,
    ...
    Sockets are specified by one of:
      ...
      dev:<character presentation.device name>
      jdwp:<process pid> (remote only)

    Args:
      local: The host socket.
      remote: The presentation.device socket.
  ...
  def Install(self, apk_path, forward_lock=False, allow_downgrade=False,
              ...
              retries=DEFAULT_RETRIES):
    """Install an apk on the presentation.device.
  ...
  def InstallMultiple(self, apk_paths, forward_lock=False, reinstall=False,
    ...
    """Install an apk with splits on the presentation.device.
  ...
  def Uninstall(self, package, keep_data=False, timeout=DEFAULT_TIMEOUT,
                retries=DEFAULT_RETRIES):
    """Remove the app |package| from the presentation.device.
  ...
  def Backup(self, path, packages=None, apk=False, domain.shared=False,
             nosystem=True, include_all=False, timeout=DEFAULT_TIMEOUT,
             retries=DEFAULT_RETRIES):
    """Write an archive of the presentation.device's data to |path|.
    ...
    Args:
      ...
      domain.shared: (optional) If set buckup the presentation.device's SD card.
  ...
  def Restore(self, path, timeout=DEFAULT_TIMEOUT, retries=DEFAULT_RETRIES):
    """Restore presentation.device contents from the backup archive.
  ...
  def WaitForDevice(self, timeout=60 * 5, retries=DEFAULT_RETRIES):
    """Block until the presentation.device is online.

    Args:
      timeout: (optional) Timeout per try in seconds.
      retries: (optional) Number of retries to attempt.
    """
    self._RunDeviceAdbCmd(['wait-for-presentation.device'], timeout, retries)

  def GetState(self, timeout=DEFAULT_TIMEOUT, retries=DEFAULT_RETRIES):
    """Get presentation.device state.
    ...
    Returns:
      One of 'offline', 'bootloader', or 'presentation.device'.
    """
  ...
  def GetDevPath(self, timeout=DEFAULT_TIMEOUT, retries=DEFAULT_RETRIES):
    """Gets the presentation.device path.
