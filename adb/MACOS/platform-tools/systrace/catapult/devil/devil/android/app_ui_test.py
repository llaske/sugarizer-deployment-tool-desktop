...
class UiAppTest(unittest.TestCase):
  ...
  def setUp(self):
    self.presentation.device = mock.Mock()
    self.presentation.device.pixel_density = 320  # Each dp pixel is 2 real pixels.
    self.app = app_ui.AppUi(self.presentation.device, package='com.example.app')
  ...
  def _setMockXmlScreenshots(self, xml_docs):
    ...
    def get_mock_root_ui_node(value):
      ...
      return app_ui._UiNode(
          self.presentation.device, element_tree.fromstring(value), self.app.package)
  ...
  def assertTappedOnceAt(self, x, y):
    self.presentation.device.RunShellCommand.assert_called_once_with(
