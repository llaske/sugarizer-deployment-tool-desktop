...
def MockDeviceWithFiles(files=None):
  ...
  presentation.device = mock.MagicMock(spec=device_utils.DeviceUtils)
  presentation.device.FileExists = mock.Mock(side_effect=file_exists)
  presentation.device.WriteFile = mock.Mock(side_effect=write_file)
  presentation.device.ReadFile = mock.Mock(side_effect=read_file)
...
class SharedPrefsTest(unittest.TestCase):
  ...
  def setUp(self):
    self.presentation.device = MockDeviceWithFiles({
  ...
  def testPropertyLifetime(self):
    prefs = shared_prefs.SharedPrefs(
        self.presentation.device, 'com.some.package', 'prefs.xml')
  ...
  def testCommit(self):
    type(self.presentation.device).build_version_sdk = mock.PropertyMock(
        return_value=version_codes.LOLLIPOP_MR1)
    prefs = shared_prefs.SharedPrefs(
        self.presentation.device, 'com.some.package', 'other_prefs.xml')
    self.assertFalse(self.presentation.device.FileExists(prefs.path))  # file does not exist
    prefs.Load()
    ...
    self.assertFalse(self.presentation.device.FileExists(prefs.path))  # still does not exist
    self.assertTrue(prefs.changed)
    prefs.Commit()
    self.assertTrue(self.presentation.device.FileExists(prefs.path))  # should exist now
    self.presentation.device.KillAll.assert_called_once_with(prefs.package, exact=True,
  ...
  def testAsContextManager_onlyReads(self):
    with shared_prefs.SharedPrefs(
        self.presentation.device, 'com.some.package', 'prefs.xml') as prefs:
      self.assertEquals(prefs.AsDict(), self.expected_data)  # loaded and ready
    self.assertEquals(self.presentation.device.WriteFile.call_args_list, [])  # did not write
  ...
  def testAsContextManager_readAndWrite(self):
    ...
    self.assertTrue(self.presentation.device.WriteFile.called)  # did write
    with shared_prefs.SharedPrefs(
        self.presentation.device, 'com.some.package', 'prefs.xml') as prefs:
      # changes persisted
