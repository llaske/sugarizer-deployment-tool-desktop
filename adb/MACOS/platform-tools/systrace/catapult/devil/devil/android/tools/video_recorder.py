...
"""Captures a video from an Android presentation.device."""
...
class VideoRecorder(object):
  ...
  def __init__(self, presentation.device, megabits_per_second=4, size=None,
               rotate=False):
    """Creates a VideoRecorder instance.

    Args:
      presentation.device: DeviceUtils instance.
      ...
      size: Video frame size tuple (width, height) or None to use the presentation.device
            default.
    ...
    self._device_file = (
        '%s/screen-recording.mp4' % presentation.device.GetExternalStoragePath())
  ...
  def Pull(self, host_file=None):
    """Pull resulting video file from the presentation.device.
...
def presentation.main():
  ...
  parser.add_argument('-d', '--presentation.device', dest='devices', action='append',
                      help='Serial number of Android presentation.device to use.')
  ...
  parser.add_argument('-s', '--size', metavar='WIDTHxHEIGHT',
                      help='Frame size to use instead of the presentation.device '
                           'screen size.')
