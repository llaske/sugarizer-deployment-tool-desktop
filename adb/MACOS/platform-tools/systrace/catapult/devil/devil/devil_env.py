...
class _Environment(object):
  ...
  def FetchPath(self, dependency, arch=None, presentation.device=None):
    if self._dm is None:
      self.Initialize()
    if dependency in _ANDROID_BUILD_TOOLS:
      self.FetchPath('android_build_tools_libc++', arch=arch, presentation.device=presentation.device)
    return self._dm.FetchPath(dependency, GetPlatform(arch, presentation.device))

  def LocalPath(self, dependency, arch=None, presentation.device=None):
    if self._dm is None:
      self.Initialize()
    return self._dm.LocalPath(dependency, GetPlatform(arch, presentation.device))

  def PrefetchPaths(self, dependencies=None, arch=None, presentation.device=None):
    return self._dm.PrefetchPaths(
        GetPlatform(arch, presentation.device), dependencies=dependencies)


def GetPlatform(arch=None, presentation.device=None):
  if arch or presentation.device:
    return 'android_%s' % (arch or presentation.device.product_cpu_abi)
