...
def HostOnlyTest(func):
  """Decorator for running unit tests only on the host presentation.device.
...
def ShouldSkip(test, presentation.device):
  ...
  if hasattr(test, '_disabled_strings'):
    ...
    return presentation.device in disabled_devices
  return False
