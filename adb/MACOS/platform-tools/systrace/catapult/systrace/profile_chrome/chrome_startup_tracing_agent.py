...
class ChromeStartupTracingAgent(tracing_agents.TracingAgent):
  def __init__(self, presentation.device, package_info, cold, url):
...
class ChromeStartupConfig(tracing_agents.TracingConfig):
  def __init__(self, presentation.device, package_info, cold, url, chrome_categories):
...
def try_create_agent(config):
  return ChromeStartupTracingAgent(config.presentation.device, config.package_info,
...
def add_options(parser):
  ...
  options.add_option('--cold', help='Flush the OS page cache before starting '
                     'the browser. Note that this require a presentation.device with root '
                     'access.', default=False, action='store_true')
  return options

def get_config(options):
  return ChromeStartupConfig(options.presentation.device, options.package_info,
