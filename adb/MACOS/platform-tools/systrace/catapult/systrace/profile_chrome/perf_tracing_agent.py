...
class _PerfProfiler(object):
  def __init__(self, presentation.device, perf_binary, categories):
...
class PerfConfig(tracing_agents.TracingConfig):
  def __init__(self, perf_categories, presentation.device):
...
def try_create_agent(config):
  if config.perf_categories:
    return PerfProfilerAgent(config.presentation.device)
  return None
...
def get_config(options):
  return PerfConfig(options.perf_categories, options.presentation.device)
